...
// diagnostic.  Included inside tb_NDP_Top_new_phy; it never drives DUT state.
    ...
    // The observer is included in tb_NDP_Top_new_phy.  Keep the progress
    // sampler on the DUT SG domain, using the same current-TB hierarchy that
    // the surrounding testbench already consumes.
    always @(posedge u_NDP_Top_new.clk_sg) begin : N75_OBS_PROGRESS
        ...
        if (n75_obs_enabled && n75_obs_feature_enabled &&
            u_NDP_Top_new.rst_n_sg) begin
