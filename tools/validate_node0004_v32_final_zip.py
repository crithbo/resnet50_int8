...
import tools.validate_node0004_v30_final_zip as v30  # noqa: E402


prior = v30.prior
INSTALL_NAME = "r5_n4_hw_v32_mse4_index_diag"
SOURCE_NAME = "r5_n4_hw_v30_mse4_descriptor_diag"
ZIP_SHA256 = "87a3e3474c3c1fbd28a8a4220919a8249c310c915da87bba58c28a7e6d8eb835"
...
FEATURES = v30.FEATURES + (
    ("RETURN_OBS_MSE4_INDEX", "+RETURN_OBS_MSE4_INDEX", "+RETURN_OBS_MSE4_INDEX_LIMIT=96"),
)
...
def feature_binding_controls(package: Path, root: Path) -> dict:
    ...
    argv_tokens = ["simv"] + [token for _, enable, limit in FEATURES for token in (enable, limit)] + [
    ...
    markers = [
        ...
        + (
            ...
            else "sample_cycles=262144 stall_windows=4 max_cycles=8388608"
            if name == "RETURN_HANG_DIAG"
            else f"limit_name={limit[1:].split('=')[0]} limit={limit.split('=')[1]}"
        )
    ...
    enable_negative = run_case([token for token in argv_tokens if token != "+RETURN_OBS_MSE4_INDEX"], markers)
    limit_negative = run_case([token for token in argv_tokens if token != "+RETURN_OBS_MSE4_INDEX_LIMIT=96"], markers)
    marker_negative = run_case(argv_tokens, [row for row in markers if "feature=RETURN_OBS_MSE4_INDEX " not in row])
...
def main() -> int:
    ...
    parser.add_argument("--source-v30", required=True, type=Path)
    ...
    source_path = args.source_v30.resolve()
    ...
    runtime_equal = runtime_paths == source_runtime_paths and all(
        prior.normalized(entries[path], INSTALL_NAME) == prior.normalized(source[path], SOURCE_NAME)
        for path in runtime_paths
    )
    changed = [
        path
        for path in sorted(set(entries) & set(source))
        if prior.normalized(entries[path], INSTALL_NAME) != prior.normalized(source[path], SOURCE_NAME)
    ]
    ...
    binding = json.loads(entries.get("provenance/current_local_rtl_binding.json", b"{}"))
    with tempfile.TemporaryDirectory(prefix="v31-final-audit-") as temp:
        root = Path(temp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(root)
        feature_controls = feature_binding_controls(root / INSTALL_NAME, root / "feature-controls")
    local_leaf_match = all(
        (ROOT / leaf["path"]).is_file() and prior.sha256_file(ROOT / leaf["path"]) == leaf["sha256"]
    ...
    checks = {
        ...
        "bound_return_report": return_report.get("valid") is True
        and return_report.get("RETURN_ANALYSIS", {}).get("return_zip", {}).get("sha256") == RETURN_SHA256,
        "manifest_exact_set_hashes": set(manifest_files) == set(entries) - {"package_manifest.json"}
        and all(path in entries and prior.sha256_bytes(entries[path]) == value for path, value in manifest_files.items()),
        "identity_classification": manifest.get("install_name") == INSTALL_NAME
        and manifest.get("classification") == "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX"
        and manifest.get("candidate_release") is False,
        "frozen_scope": manifest.get("numeric_analysis_repeated") is False
        ...
        and runtime_equal,
        "only_expected_source_changes": set(changed) == expected_changed and not added,
        "current_rule_receipts": all(value in receipt_text for value in CURRENT_RECEIPTS.values()),
        "observer_narrow_feature": observer.count("MSE4_INDEX_BOUNDARY_V1") == 1
        and observer.count("feature=RETURN_OBS_MSE4_INDEX enabled=%0d") == 1
        and "return_obs_write_mse4_index_state(event_name);" in observer
        and "MSE4_INDEX_EDGE_V1" in observer,
        "actual_runner_binding": prepare.count("+RETURN_OBS_MSE4_INDEX") == 4
        and prepare.count("+RETURN_OBS_MSE4_INDEX_LIMIT=96") == 2
        and "+define+NATIVE_RETURN_OBSERVER_ENABLE" in prepare
        and "+incdir+$package_root/tb_probe" in prepare,
        "collector_binding": runtime.count('"feature": "RETURN_OBS_MSE4_INDEX"') == 1
        and runtime.count('"enable": "+RETURN_OBS_MSE4_INDEX"') == 1
        and runtime.count('"limits": ("+RETURN_OBS_MSE4_INDEX_LIMIT=96",)') == 1,
    ...
    report = {
        "schema": "node0004-v32-final-zip-rule-self-audit-v1",
        "valid": not errors,
        ...
        "source_v30_sha256": prior.sha256_file(source_path),
