#!/usr/bin/env python3
"""Create the exact final-ZIP release receipt for native-four-lane p23."""
...
BASE = ROOT / "outputs/conv_native_four_lane_0ccae916_p23_epochflow"
ZIP_PATH = BASE / "build_v2" / f"{PACKAGE_ID}.zip"
BUILD = BASE / "build_v2" / f"{PACKAGE_ID}.build.json"
FAMILY = BASE / "p23_family_audit_v3.json"
HARNESS = BASE / "p23_runtime_layout_harness_v2.json"
SHARED = BASE / "p23_shared_runtime_layout_v2.json"
PROFILE = BASE / "server_package_build_profile_v2.json"
ANALYSIS = ROOT / "outputs/conv_native_four_lane_0ccae916_p22_return_analysis/report_v2.json"
...
def main() -> int:
    ...
    if not all(path.is_file() for path in required):
        raise FinalizeError("required p23 release input is absent")
    ...
    scope = family["observer"]["focused_compile"]["p23_epoch_flow"]
    runtime_ok = all(
    ...
    memory_identity = (
        manifest["expected_production_rtl_identity"]["leaves"].get("Memory_AG_Idx_Queue.sv")
        == "b555ab22523540a9aa49d3eb51dee6eea9962086a71429028c69964de3819989"
        and manifest["cloud_rtl_authority"]["leaves"].get("Memory_AG_Idx_Queue.sv")
        == "b555ab22523540a9aa49d3eb51dee6eea9962086a71429028c69964de3819989"
        and runtime.count('"Memory_AG_Idx_Queue.sv"') == 2
    )
    checks = {
        "exact_zip_identity": ZIP_PATH.stat().st_size == build["zip_bytes"] and sha256(ZIP_PATH) == build["zip_sha256"],
        "formal_p22_analysis": analysis["valid"] is True and analysis["status"] == "P22_EO_FIX_PASS_EPOCH_FLOW_SUCCESSOR_REQUIRED",
        ...
        "private_xmr_scope": scope["valid"] is True and scope["positive"]["exit_code"] == 0 and scope["negative_leaf_deleted"]["exit_code"] != 0 and scope["negative_leaf_renamed"]["exit_code"] != 0 and scope["negative_wrong_sibling_path"]["exit_code"] != 0,
        "diagnostic_predicate_trace": scope["predicate_trace"]["valid"] is True and scope["predicate_trace"]["stable_level_emits_transaction"] is False,
        "production_memory_ag_identity_collector": memory_identity,
        ...
        "observer_and_runner_binding": (
            observer.count("p23 EPOCH_FLOW_ACTUAL_CONSUMER_BEGIN") == 1
            and runner.count("+RETURN_OBS_EPOCH_FLOW +RETURN_OBS_EPOCH_FLOW_LIMIT=256") == 2
            and "+RETURN_OBS_EPOCH_OWNER" not in runner
        ),
    ...
    result = {
        "schema": "conv-native-four-lane-0ccae916-p23-final-zip-audit-v1",
        ...
        "audits": {
            "p22_return_analysis": receipt(ANALYSIS), "build_profile": receipt(PROFILE),
            "build": receipt(BUILD),
            ...
            "shared_runtime_layout": {**receipt(SHARED), "pass": shared["pass"], "errors": len(shared["errors"]), "exact_final_zip_invocation_count": 1},
        },
        "frozen_surface": {
        ...
        "release_gate_matrix": {
            ...
            "package_local_hdl": {"applicability": "blocking_applicable", "pass": checks["family_audit"] and checks["private_xmr_scope"] and checks["production_memory_ag_identity_collector"]},
            "diagnostic_semantics": {"applicability": "blocking_applicable", "pass": checks["diagnostic_predicate_trace"]},
        ...
        "claim_boundary": "p23 is one c0 diagnostic. It does not claim c0 natural terminal, 27 natural terminals, formal 320D, E3/E4/E5, numeric correctness or performance before formal server return.",
        "server_action": False,
    }
    if OUTPUT.exists():
        raise FinalizeError("refusing to overwrite p23 final audit")
