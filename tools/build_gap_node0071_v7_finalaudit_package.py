...
INSTALL_NAME = "r5_n71_gap_v7_finalaudit"
SOURCE_NAME = "r5_n71_gap_v6_canonical"
SOURCE_ZIP = (
    ROOT
...
SOURCE_SHA256 = (
    "aeb92c6f6442fa6e04f9207b791ccc4bab32b5ac1584b425c4cc3945f2dbdc38"
)
OUTPUT_ROOT = (
    ROOT
    / "artifacts/operator_config_validation/r5-server-test-packages"
...
def extract_source(destination: Path) -> Path:
    if sha256(SOURCE_ZIP) != SOURCE_SHA256:
        raise BuildError("frozen v6 source differs")
    package = destination / INSTALL_NAME
    package.mkdir(parents=True, exist_ok=False)
    with zipfile.ZipFile(SOURCE_ZIP) as archive:
        if archive.testzip() is not None:
            raise BuildError("frozen v6 source CRC differs")
...
def build_directory(destination: Path) -> tuple[Path, dict[str, Any]]:
    ...
    (package / "README.md").write_text(
        "# GAP node0071 v7 current-rule final audit\n\n"
        "This package is `DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX`. It reuses "
        "the frozen v6 diagnostic workload and adds the current-rule "
        "final-ZIP self-audit contract. Run once with:\n\n"
        "```bash\n"
    ...
    manifest.update(
        {
            "schema": "gap-node0071-progress-server-package-v7",
            "status": "PACKAGE_READY_NOT_RUN",
            "claim": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX",
            "package_class": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX",
            "claim_boundary": (
                "current-rule final-ZIP receipt only; frozen GAP "
                "sum/tail/golden/config/workload and v6 diagnostic behavior "
                "unchanged; no functional fix and no E3/E4/E5"
            ),
    ...
    provenance.update(
        {
            "tool":
                "tools/build_gap_node0071_v7_finalaudit_package.py",
            "bound_source_package_sha256": SOURCE_SHA256,
            "numeric_payload_rebuilt": False,
            "diagnostic_only": True,
            "package_side_change":
                "current-rule final-ZIP self-audit receipt only",
        }
    )
...
def main() -> int:
    ...
    try:
        ...
        validation = {
            "schema":
                "gap-node0071-current-rule-package-validation-v7",
        ...
        print(f"GAP v7 build failed: {error}", file=sys.stderr)
        return 1
