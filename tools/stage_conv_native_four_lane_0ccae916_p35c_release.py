#!/usr/bin/env python3
"""Stage immutable p35c ZIP and receipts for family rotation."""
...
SOURCES = {
    ...
    f"{PACKAGE}.family_audit.json": BASE / "p35c_family_audit.json",
    ...
    f"{PACKAGE}.first_fresh_contract.json": ROOT / "outputs/p35c_first_fresh_audit_v2/contract.json",
    f"{PACKAGE}.first_fresh_validation.json": ROOT / "outputs/p35c_first_fresh_audit_v2/validation.json",
}


def main() -> int:
    if STAGE.exists():
        raise RuntimeError("refusing to overwrite p35c release stage")
    if not all(path.is_file() for path in SOURCES.values()):
        raise RuntimeError("p35c release evidence is absent")
