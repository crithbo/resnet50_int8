...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the full matrix-complete node-0004 requant candidate."
    )
    ...
    semantic_output = (
        ...
        if args.semantic_output.is_absolute()
        else ROOT / args.semantic_output
    )
    if semantic_output.exists():
        print(
            f"refusing to overwrite semantic contract: {semantic_output}",
            file=sys.stderr,
        )
        return 1
    try:
        value = build_requant_server_candidate(ROOT, output)
        semantic_output.parent.mkdir(parents=True, exist_ok=True)
        semantic_output.write_bytes((output / "semantic_contract.json").read_bytes())
    except Exception as error:
        print(f"requant server candidate generation failed: {error}", file=sys.stderr)
        return 1
    print(
        json.dumps(
            {
