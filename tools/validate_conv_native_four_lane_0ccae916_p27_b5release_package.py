#!/usr/bin/env python3
"""Family audit for the p27 generated source-bound Buffer5 diagnostic."""
...
import validate_conv_native_four_lane_0ccae916_p26_memag_package as previous
...
def parser_trace(parser_bytes: bytes) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="p27_parser_trace_") as temporary:
        root = Path(temporary)
    ...
    return {
        "schema": "conv-native-four-lane-p27-source-bound-parser-trace-v1",
        "cases": results,
...
def main() -> int:
    ...
    checks = {
        ...
        "legacy_observer_byte_equal_p26": base.digest(legacy_observer) == LEGACY_OBSERVER_SHA256,
    ...
    report.update(
        {
            "schema": "conv-native-four-lane-p27-source-bound-family-audit-v1",
