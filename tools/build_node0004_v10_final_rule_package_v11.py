...
INSTALL_NAME = "r5_n4_hw_v11_hangloc_finalaudit"
SOURCE_INSTALL_NAME = "r5_n4_hw_v10_hangloc_canonical"
SOURCE_ZIP = (
    ROOT
    / "artifacts/operator_config_validation/r5-server-test-packages/"
    "r5_n4_hw_v10_hangloc_canonical.zip"
)
SOURCE_ZIP_SHA256 = (
...
def _configure_base() -> None:
    ...
    base.RUNTIME_SOURCE = (
        ROOT / "tools/node0004_hang_localization_runtime_v11.py"
    )
...
def _readme() -> str:
    return f"""# node0004 v11 final-rule-audited hang-localization package

Classification: `DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX`.

This package reuses the frozen v10 c0 workload without rebuilding node0004
numeric inputs. It preserves qualified-only progress and the complete,
versioned canonical decision record. The only executable change is setting
`sys.dont_write_bytecode=True` before importing the package-local runtime
helper, as required by the bootstrap-immutability rule.
...
def build_directory(destination: Path) -> tuple[Path, dict[str, Any]]:
    ...
    manifest["schema"] = "resnet50-node0004-hang-localization-package-v11"
    manifest["install_name"] = INSTALL_NAME
    manifest["evidence_level"] = (
        "E2_LOCAL_PLUS_V10_FINAL_RULE_SELF_AUDIT_REPAIR"
    )
    ...
    manifest["package_side_repair"] = {
        ...
        "first_defect": (
            "v10 package runtime imported a package-local helper before "
            "setting sys.dont_write_bytecode=True"
        ),
        "second_defect": (
            "the defect was found by the post-generation current-rule "
            "final-ZIP self-audit before server release"
        ),
        "repair": (
            "set sys.dont_write_bytecode=True before importing the preserved "
            "package-local v7 helper; retain shell PYTHONDONTWRITEBYTECODE=1"
        ),
    ...
    manifest["unresolved_boundary"] = (
        ...
        "and before any Buffer5 write witness; v11 changes only bootstrap "
        "immutability and post-generation rule receipts"
    )
    ...
    manifest["superseded_diagnostic_package"] = {
        "name": "r5_n4_hw_v10_hangloc_canonical.zip",
        "sha256": SOURCE_ZIP_SHA256,
        "status": "QUARANTINED_BOOTSTRAP_IMMUTABILITY_DEFECT",
    }
...
def _repeat(package: Path, zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="node0004-v11-repeat-") as temporary:
...
def main() -> int:
    ...
    receipt = {
        "schema": "node0004-hang-localization-package-validation-v11",
