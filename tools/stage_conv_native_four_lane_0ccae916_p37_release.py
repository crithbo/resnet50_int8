#!/usr/bin/env python3
"""Stage immutable p37 ZIP and evidence for one storage rotation."""
...
SOURCES = {
    ...
    f"{PACKAGE}.family_audit.json": BASE / "p37_family_audit.json",
...
def main() -> int:
    if STAGE.exists():
        raise RuntimeError("refusing to overwrite p37 release stage")
    if not all(path.is_file() for path in SOURCES.values()):
        raise RuntimeError("p37 release evidence is absent")
