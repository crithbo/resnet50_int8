...
SOURCE_ROOT = "r5_n71_gap_v28_ga_mse4_final_pair_diag"
...
def configure() -> None:
    ...
    audit.EXTRA_VALIDATORS = [
        (
            "mse0_buffer_prep_group0_validator_and_controls",
            ROOT / "tools/validate_gap_node0071_mse0_buffer_prep_group0_v29.py",
            [
                str(audit.ZIP),
                "--root-name",
                ROOT_NAME,
...
def main() -> int:
    ...
    signal_checks = {
        ...
        "mse0_feature_all_bound": all(
            signal.get("return", {})
            .get("mse0_buffer_prep_group0_binding", {})
            .values()
        ),
    ...
    hdl_checks = {
        ...
        "projection_compile_zero":
            positive.get("projection_compile", {}).get("exit_code") == 0,
    ...
    if not all(hdl_checks.values()):
        result["errors"].append("focused v29 package-local HDL gate differs")
    if result["errors"]:
    ...
    result.update(
        {
            "schema": "gap-node0071-v29-mse0-buffer-prep-group0-final-audit-v1",
            "source_v28": source_receipt,
            ...
            "diagnostic_contract": {
                "test_id":
                    "r5-gap-node0071-v29-mse0-buffer-prepared-group0-diagnostic",
                "classification": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX",
                "last_proven_good": (
                    "both producers reach buffers 13 times; all 48 GA accepts "
                    "retire and MSE4 consumes 12 available write-data beats"
                ),
                "first_divergence": (
