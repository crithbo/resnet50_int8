#!/usr/bin/env python3
"""Exact-ZIP frontend/scope gate for the guarded v99 targeted observer."""
...
def main() -> int:
    ...
    if "always @(sig_clk or" in source or "force_all || sig_clk !== prev_sig_clk" in source:
        errors.append("redundant_every_clock_edge_transport_present")
    ...
    with tempfile.TemporaryDirectory(prefix="node0004-v99-hdl-") as temporary:
        root = Path(temporary)
    ...
    report = {
        "schema": "node0004-v99b-lcdup-guarded-hdl-gate-v1",
        "package_id": PACKAGE,
        "pass": not errors,
        "errors": errors,
        ...
        "claim_boundary": "Exact-final-ZIP module frontend and targeted bind/catalog closure; production VCS elaboration and guard behavior remain dynamic proof boundaries.",
    }
