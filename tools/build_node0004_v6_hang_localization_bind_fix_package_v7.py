...
INSTALL_NAME = "r5_n4_hw_v7_hangloc_bind"
SOURCE_INSTALL_NAME = "r5_n4_hw_v6_hangloc"
SOURCE_ZIP = (
    ROOT
    / "artifacts/operator_config_validation/r5-server-test-packages/"
    "r5_n4_hw_v6_hangloc.zip"
)
SOURCE_ZIP_SHA256 = (
    "2a0ecf7e0218a2a65d37d281ef46343f66e20ca4359cfacf062bf88f89dd1021"
)
RETURN_ZIP_SHA256 = (
...
def _extract_c0_only(destination: Path) -> Path:
    if sha256(SOURCE_ZIP) != SOURCE_ZIP_SHA256:
        raise BuildError("frozen v6 source ZIP identity differs")
    ...
    with zipfile.ZipFile(SOURCE_ZIP) as archive:
        if archive.testzip() is not None:
            raise BuildError("frozen v6 source ZIP CRC failed")
...
def _readme() -> str:
    return f"""# node0004 v7 observer-bind repair package

Classification: `DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX`.

This package is not a Conv result candidate and cannot establish E4/E5. It
repairs the v6 package-side compile binding by defining
`NATIVE_RETURN_OBSERVER_ENABLE` together with the package-local observer include
directory. It preserves the frozen v6 c0 workload and stops on either natural
c0 terminal, four consecutive 262144-cycle no-progress windows, or an
8388608-cycle diagnostic budget. It does not modify functional RTL or the
server TB.
...
def build_directory(destination: Path) -> tuple[Path, dict[str, Any]]:
    ...
    manifest = {
        "schema": "resnet50-node0004-hang-localization-package-v7",
        "install_name": INSTALL_NAME,
        "classification": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX",
        "status": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX_READY_NOT_RUN",
        "candidate_release": False,
        "evidence_level": "E2_LOCAL_PLUS_V6_PACKAGE_DIAGNOSTIC_FAILURE",
        "run_ids": ["c0"],
        ...
        "package_side_repair": {
            ...
            "first_divergence": (
                "v6 compile command supplied only the observer include "
                "directory and omitted +define+NATIVE_RETURN_OBSERVER_ENABLE"
            ),
            "v6_observer_bytes": 0,
            ...
            "repair": (
                "add +define+NATIVE_RETURN_OBSERVER_ENABLE to VCS_EXTRA_OPTS; "
                "keep package-local +incdir and all functional inputs unchanged"
            ),
...
def _repeat(package: Path, zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="node0004-v7-repeat-") as temporary:
...
def main() -> int:
    ...
    try:
        ...
        receipt = {
            "schema": "node0004-hang-localization-package-validation-v7",
            "status": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX_READY_NOT_RUN",
            "package": str(package),
            "zip": str(zip_path),
