#!/usr/bin/env python3
"""Stage the immutable corrected p37b ZIP and exact audit receipts."""
...
SOURCES = {
    ...
    f"{PACKAGE}.runner_harness.json": BUILD / f"{PACKAGE}.runner_harness_v2.json",
    f"{PACKAGE}.shared_layout.json": BUILD / f"{PACKAGE}.shared_layout_v2.json",
    ...
    f"{PACKAGE}.family_audit.json": BASE / "p37b_family_audit.json",
...
def main() -> int:
    if STAGE.exists():
        raise RuntimeError("refusing to overwrite p37b release stage")
    if not all(path.is_file() for path in SOURCES.values()):
        raise RuntimeError("p37b release evidence is absent")
