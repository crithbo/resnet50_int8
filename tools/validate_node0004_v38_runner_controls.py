...
import tools.validate_node0004_v37_runner_controls as prior  # noqa: E402
...
def write_stubs(stub_root: Path, python: Path) -> None:
    ...
    anchor = (
        ...
        "feature=RETURN_OBS_WRDRAIN enabled=1 "
        "limit_name=RETURN_OBS_WRDRAIN_LIMIT limit=1\n"
    )
    addition = anchor + (
        "0 | DIAGNOSTIC_FEATURE_ENABLE_V1 | "
        "feature=RETURN_OBS_WRTERM enabled=1 "
        "limit_name=RETURN_OBS_WRTERM_LIMIT limit=96\n"
    )
    if text.count(anchor) != 1:
        raise ValueError("safe-stub v38 marker anchor differs")
    make.write_text(
...
def main() -> int:
    ...
    report = base.validate(
        ...
        report_schema="node0004-v38-runner-controls-v1",
