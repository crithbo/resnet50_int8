#!/usr/bin/env python3
"""Finalize the immutable p37 ZIP after all exact-ZIP gates pass."""
...
BASE = ROOT / "outputs/conv_native_four_lane_0ccae916_p37_saepoch"
BUILD = BASE / "build"
...
FILES = {
    ...
    "family": BASE / "p37_family_audit.json",
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError("refusing to overwrite p37 final audit")
    if not ZIP.is_file() or not all(path.is_file() for path in FILES.values()):
        raise RuntimeError("p37 final evidence is incomplete")
    ...
    checks = {
        ...
        "live_causal_required_plugins": live.get("contract_errors") == [] and set(plugin_results) == {"arm_known_parser", "sa_epoch_parser"} and all(row.get("executed") is True and row.get("pass") is True for row in plugin_results.values()),
        ...
        "p36b_formal_analysis": reports["p36b_return_analysis"].get("valid") is True and reports["p36b_return_analysis"].get("status") == "P36B_PARTIAL_RETURN_VALID_STABLE_ARM_TAG_REACCEPT_PROVEN_SUCCESSOR_REQUIRED",
    }
    ...
    matrix = {
        ...
        "diagnostic_semantics": {"applicability": "blocking_applicable", "pass": checks["semantic_controls"], "fingerprint_sha256": fingerprint, "disposition": "FIRST_USE_AUDITED_BY_TYPED_V2_FINAL_ZIP"},
    ...
    result = {
        "schema": "conv-native-four-lane-p37-saepoch-final-zip-audit-v1",
        ...
        "claim_boundary": "One c0 diagnostic distinguishing complete accepted SA data beats behind the p36b stable ARM tag. No natural terminal, formal 320D, E3/E4/E5 or measured performance claim.",
        "server_action": False,
    }
