...
def shaf(path: Path) -> str: return sha(path.read_bytes())
def load(path: Path): return json.loads(path.read_text(encoding="utf-8"))

def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--zip", required=True, type=Path); ap.add_argument("--sidecar", required=True, type=Path)
    ap.add_argument("--source-v67", required=True, type=Path); ap.add_argument("--build-report", required=True, type=Path)
    ap.add_argument("--family-report", required=True, type=Path); ap.add_argument("--shared-report", required=True, type=Path)
    ap.add_argument("--observer-report", required=True, type=Path); ap.add_argument("--runner-report", required=True, type=Path)
    ap.add_argument("--return-report", required=True, type=Path); ap.add_argument("--output", required=True, type=Path)
    a = ap.parse_args(); checks = {}; zsha = shaf(a.zip)
    checks["source_identity"] = shaf(a.source_v67) == SOURCE_SHA
    ...
    with zipfile.ZipFile(a.zip) as z, zipfile.ZipFile(a.source_v67) as source_zip:
        ...
        checks["safe_paths"] = all(not PurePosixPath(i.filename).is_absolute() and ".." not in PurePosixPath(i.filename).parts and "\\" not in i.filename for i in infos)
        ...
        source = {"/".join(i.filename.split("/")[1:]): source_zip.read(i) for i in source_zip.infolist() if not i.is_dir()}
        manifest = json.loads(target["package_manifest.json"]); actual = {p: sha(b) for p, b in target.items() if p != "package_manifest.json"}
        checks["manifest_exact_set"] = set(manifest["files"]) == set(actual)
        checks["manifest_receipts"] = manifest["files"] == actual
        checks["manifest_identity"] = manifest["install_name"] == PACKAGE and manifest["source_package_sha256"] == SOURCE_SHA and manifest["classification"] == "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX" and manifest["candidate_release"] is False and manifest["configuration_rebuilt"] is False and manifest["node0004_workload_rebuilt"] is False and manifest["numeric_analysis_repeated"] is False and manifest["functional_rtl_modified"] is False and manifest["server_action"] is False
        feature = manifest["diagnostic_features"]["RETURN_OBS_PE7_PAIR"]
        checks["feature_contract"] = feature["runtime_enable_parameter"] == "+RETURN_OBS_PE7_PAIR" and feature["limit_parameter"] == "+RETURN_OBS_PE7_PAIR_LIMIT=128" and feature["edge_schema"] == "PE7_PAIR_V1" and feature["logical_to_physical_binding"]["logical_PE1"] == "physical_PE7"
        runner = target["PREPARE_AND_RUN.sh"].decode()
        checks["feature_actual_argv"] = runner.count(" +RETURN_OBS_PE7_PAIR +RETURN_OBS_PE7_PAIR_LIMIT=128") == 2
        checks["install_only_v2"] = json.loads(target["SERVER_RUNTIME_LAYOUT_CONTRACT.json"])["required_preexisting_parents"] == ["install"]
        checks["fresh_provenance"] = "provenance/v67_to_v68_pe7_pair.json" in target
        frozen = [p for p in source if (p.startswith("workload/") or "golden" in p.lower() or p.endswith(".bin")) and p in target]
        checks["frozen_payload"] = bool(frozen) and all(target[p].replace(PACKAGE.encode(), source_root.encode()) == source[p] for p in frozen)
    ...
    report = {"schema": "node0004-v68-final-zip-audit-v1", "FINAL_ZIP_RULE_SELF_AUDIT_PASS": not errors,
              "errors": errors, "checks": checks,
              "zip": {"path": str(a.zip.resolve()), "bytes": a.zip.stat().st_size, "sha256": zsha},
              "current_rule_receipts": {key: {"path": str(path), "sha256": shaf(path)} for key, path in RULES.items()},
              "release_gate_matrix": matrix,
              "claim_boundary": "Exact final ZIP, corrected physical PE7 pair observer, runner/layout/finalizer and result gate only; no DUT natural terminal/formal320D/E4/E5 claim."}
    a.output.parent.mkdir(parents=True, exist_ok=True)
    a.output.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8", newline="\n")
    print(json.dumps({"pass": not errors, "errors": errors, "sha": zsha})); return 0 if not errors else 1
