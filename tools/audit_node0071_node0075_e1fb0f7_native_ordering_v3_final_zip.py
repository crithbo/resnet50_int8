#!/usr/bin/env python3
"""Independent final-ZIP audit for the node0071 -> node0075 v3 package."""
...
BUILD_REPORT = (
    ...
    "r5-node0071-node0075-e1fb0f7-native-ordering-package-v3/"
    "build_report.json"
)
...
def closure(source: str) -> dict[str, Any]:
    ...
    always_begin = source.index("    always @(posedge clk_sg)")
    final_begin = source.index("    final begin : N75_OBS_FINAL")
...
def hdl_gate(package: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "Focused syntax/name-resolution and exact canonical/result consumer "
            "closure only; external DUT XMR assignments specialized; production "
            "VCS remains the full-design compile/elaboration evidence."
        ),
        "pass": valid,
    }
...
def validate(zip_path: Path, sidecar: Path) -> dict[str, Any]:
    ...
    return {
        "schema":
            "node0071-node0075-native-ordering-v3-final-zip-self-audit-v1",
