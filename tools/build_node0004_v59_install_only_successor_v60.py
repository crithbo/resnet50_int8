...
SOURCE_SHA = "e5023a50e827ae3d4b0fc6bb9ac327c9aa38d9e72db068cc4fd567f8e76a216d"
SOURCE_ZIP = (
    ROOT
    / "artifacts/operator_config_validation/r5-server-test-packages/superseded"
    / "conv_serialized_node0004"
    / SOURCE
    / f"{SOURCE}.zip"
)
DEFAULT_OUTPUT = (
    ROOT / "outputs/conv_node0004_v59_install_only_successor/v60_build"
)
SERVER_RULE_SHA = (
...
def extract_source(destination: Path) -> Path:
    if sha256(SOURCE_ZIP) != SOURCE_SHA:
        raise BuildError("v59 source ZIP SHA differs")
    with zipfile.ZipFile(SOURCE_ZIP) as archive:
        bad = archive.testzip()
        if bad is not None:
            raise BuildError(f"v59 source CRC failed at {bad}")
        roots: set[str] = set()
        seen: set[str] = set()
        ...
        if roots != {SOURCE}:
            raise BuildError(f"v59 source root differs: {sorted(roots)}")
...
def update_manifest(package: Path, contract: dict[str, object]) -> None:
    ...
    manifest["v59_hold_adjudication"] = {
...
def build_directory(output: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="node0004-v59-source-") as temp:
    ...
    write_json(
        package / "provenance/v59_to_v60_install_only.json",
        {
            "schema": "node0004-v59-to-v60-install-only-v1",
            "source_v59_sha256": SOURCE_SHA,
    ...
    (package / "README.md").write_text(
        "# node0004 v60 install-only runtime-layout fix\n\n"
        "Classification: `DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX`.\n\n"
        "This fresh successor replaces held v59 mechanically. Only "
        "`$server_root/install` must pre-exist as a real non-symlink "
        "directory. The package safely creates missing shared `cfg_pkg` and "
        "`codex_runs` directories below it; "
...
def main() -> int:
    ...
    if any(path.exists() for path in targets):
        raise BuildError("refusing to overwrite existing v60 target")
    package = build_directory(output)
    zip_path = output / f"{INSTALL}.zip"
    deterministic_zip(package, zip_path)
    digest = sha256(zip_path)
    with tempfile.TemporaryDirectory(prefix="node0004-v60-repeat-") as temp:
    ...
    if not deterministic:
        raise BuildError("v60 deterministic rebuild differs")
    sidecar = output / f"{INSTALL}.zip.sha256"
    sidecar.write_text(
        f"{digest}  {zip_path.name}\n", encoding="ascii", newline="\n"
    )
    report = {
        "schema": "node0004-v59-to-v60-install-only-build-v1",
