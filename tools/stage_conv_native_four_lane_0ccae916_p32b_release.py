#!/usr/bin/env python3
"""Stage immutable p32b ZIP and receipts for family-scoped rotation."""
...
SOURCES = {
    ...
    f"{PACKAGE}.family_audit.json": BASE / "p32b_family_audit.json",
...
def main() -> int:
    if STAGE.exists():
        raise RuntimeError("refusing to overwrite p32b release stage")
    if not all(path.is_file() for path in SOURCES.values()):
        raise RuntimeError("p32b release evidence is absent")
