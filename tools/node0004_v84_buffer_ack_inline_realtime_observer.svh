`timescale 1ns/1ps
module codex_probe_buf_ack_inline_realtime(
  ...
  localparam integer CODEX_PAYLOAD_WIDTH = 42;
  ...
  wire [1:0] codex_expected_bp = {2{!buf_ag_idx_queue_full}} & buf_idx_bp_pre_mask;
  wire [1:0] codex_bp_xor = mse_buf_queue_bp_pre ^ codex_expected_bp;
  wire [CODEX_PAYLOAD_WIDTH-1:0] codex_payload = {
    ...
    mse_buf_queue_row_tag, mse_buf_queue_col_tag, codex_expected_bp, codex_bp_xor
  };
  ...
  reg [1:0] codex_pre_expected, codex_pre_xor;
  ...
  time codex_pre_time;
  realtime codex_pre_realtime;

  task automatic codex_emit_live(input [127:0] phase_name, input integer phase_ord, input integer seq_value);
    $display("CODEX_PROBE_V1 kind=EVENT boundary=buf_ack_inline_realtime_target instance=%0s time=%0t rt=%0.3f mask=1 payload=%0h payload_known=%0d payload_width=%0d seq=%0d phase=%0s ord=%0d wr=%b full=%b all=%b valid=%h same=%h gotten=%h keep=%h bpmask=%h bp=%h mode=%h row=%h col=%h rowtag=%h coltag=%h expected=%h xor=%h",
      CODEX_TARGET, $time, $realtime, codex_payload, !$isunknown(codex_payload), CODEX_PAYLOAD_WIDTH,
      seq_value, phase_name, phase_ord, buf_ag_idx_queue_wr_en, buf_ag_idx_queue_full,
      ...
      codex_expected_bp, codex_bp_xor);
  endtask

  task automatic codex_emit_pre(input integer seq_value);
    $display("CODEX_PROBE_V1 kind=EVENT boundary=buf_ack_inline_realtime_target instance=%0s time=%0t rt=%0.3f mask=1 payload=%0h payload_known=%0d payload_width=%0d seq=%0d phase=PRE ord=0 wr=%b full=%b all=%b valid=%h same=%h gotten=%h keep=%h bpmask=%h bp=%h mode=%h row=%h col=%h rowtag=%h coltag=%h expected=%h xor=%h",
      CODEX_TARGET, codex_pre_time, codex_pre_realtime, codex_pre_payload, !$isunknown(codex_pre_payload), CODEX_PAYLOAD_WIDTH,
      ...
      codex_pre_rowtag, codex_pre_coltag, codex_pre_expected, codex_pre_xor);
  endtask
  ...
  initial begin
    ...
    if (codex_enabled)
      $display("CODEX_PROBE_V1 kind=ENABLED boundary=buf_ack_inline_realtime_target instance=%0s feature=RETURN_OBS_BUF_ACK_INLINE_REALTIME limit=%0d payload_width=%0d",
  ...
  always @(negedge clk) begin
    if (!rst_n) codex_pre_valid = 0;
      ...
      #0.250;
      codex_pre_payload = codex_payload; codex_pre_time = $time; codex_pre_realtime = $realtime;
      ...
      codex_pre_expected = codex_expected_bp; codex_pre_xor = codex_bp_xor;
      codex_pre_valid = 1;
    end
  end

  always @(posedge clk) begin : codex_inline_realtime_capture
    integer seq_value;
    if (rst_n && codex_enabled && codex_pre_valid && codex_count < codex_limit &&
      ...
      codex_emit_live("EDGE", 1, seq_value);
      #0.001 codex_emit_live("DELTA_1PS", 2, seq_value);
      #0.249 codex_emit_live("QUARTER_250PS", 3, seq_value);
      #0.500 codex_emit_live("LATE_750PS", 4, seq_value);
    end
  end

  final if (codex_enabled)
    $display("CODEX_PROBE_V1 kind=SUMMARY boundary=buf_ack_inline_realtime_target instance=%0s count=%0d state=0 first=0 last=%0t maxgap=0 sticky=0 xor=0 payload_width=%0d",
      CODEX_TARGET, codex_count, $time, CODEX_PAYLOAD_WIDTH);
endmodule

`ifndef CODEX_SOURCE_BOUND_FOCUS
bind tb_NDP_Top_new_phy.u_NDP_Top_new.slice_with_datahub_mc_group_gen[13].u_slice_with_datahub_mc_group.slice_group_gen[1].u_slice_wrapper.u_Slice.u_LSU.u_Stream_Engine.MSE_INST[4].WR_MSE.u_Memory_WR_Stream_Engine.u_Buffer_AG_Idx_Queue codex_probe_buf_ack_inline_realtime codex_probe_buf_ack_inline_realtime_inst (
  .clk(clk), .rst_n(rst_n), .buf_ag_idx_queue_wr_en(buf_ag_idx_queue_wr_en),
  .buf_ag_idx_queue_full(buf_ag_idx_queue_full), .buf_all_idx_matched(buf_all_idx_matched),
  .buf_idx_valid_bit_masked(buf_idx_valid_bit_masked), .buf_idx_same_bit_masked(buf_idx_same_bit_masked),
  .buf_idx_gotten_bit(buf_idx_gotten_bit), .buf_idx_bp_pre_keep_mask(buf_idx_bp_pre_keep_mask),
  .buf_idx_bp_pre_mask(buf_idx_bp_pre_mask), .mse_buf_queue_bp_pre(mse_buf_queue_bp_pre),
  .mse_buf_idx_mode(mse_buf_idx_mode), .mse_buf_queue_row_idx(mse_buf_queue_row_idx),
