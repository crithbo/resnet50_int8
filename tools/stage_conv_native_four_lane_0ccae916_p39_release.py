#!/usr/bin/env python3
"""Stage p39 release receipts next to its immutable ZIP for rotation."""
...
def main() -> int:
    if not all(path.is_file() for path in SOURCES.values()):
        raise RuntimeError("p39 release evidence is incomplete")
    targets = [BUILD / name for name in SOURCES]
    if any(path.exists() for path in targets):
        raise RuntimeError("refusing to overwrite p39 staged receipt")
