...
def main() -> int:
    ...
    actual_time0_binding = (
        ...
        and "mapping=logical13_14_15_physical6_8_17" in observer_log
    )
    ...
    if not all(
        (
            ...
            compile_invoked,
            actual_time0_binding,
            formal_feature_binding,
            argv_complete,
        )
    ):
        errors.append("preflight/compile/dynamic feature binding differs")

    canonical = kv(observer_log, "CANONICAL_DIAG_DECISION_V1")
    chain = kv(observer_log, "LC13_LC14_BOUNDARY_V1")
    dterm = kv(observer_log, "DTERM_OWNER_BOUNDARY_V1")
    lc18_edges = [
        dict(re.findall(r"([A-Za-z0-9_]+)=([^ ]+)", line))
        for line in observer_log.splitlines()
        if "LC18_PE7_EDGE_V1" in line
    ]
    final_lc18 = lc18_edges[-1] if lc18_edges else {}

    # LC port is {valid,last,same,last_index[3:0],data[15:0]}.
    lc17_port = number(final_lc18, "lc17_port", 0)
    lc17_valid = (lc17_port >> 22) & 1
    lc17_last = (lc17_port >> 21) & 1
    lc17_same = (lc17_port >> 20) & 1
    lc17_last_index = (lc17_port >> 16) & 0xF
    lc17_bp = number(final_lc18, "lc17_bp", 0)
    lc17_to_pe7_ready = (lc17_bp >> 18) & 1

    final_json = json.loads(FINAL_JSON_PATH.read_text(encoding="utf-8"))
    pe1 = final_json["lc_pe_configs"]["PE1"]
    config_exact = (
        pe1["inport0"]["src_id"] == "DRAM_LC.LC15"
        and pe1["inport0"]["mode"] == "keep"
        and pe1["inport0"]["keep_last_index"] == 2
        and pe1["inport2"]["src_id"] == "DRAM_LC.LC9"
        and pe1["inport2"]["mode"] == "buffer"
    )
    ...
    mapping_exact = (
        node_to_resource.get("DRAM_LC.LC15") == "LC17"
        and node_to_resource.get("DRAM_LC.LC9") == "LC18"
        and node_to_resource.get("LC_PE.PE1") == "PE7"
    )

    dynamic = {
        "mapped_chain_advanced": (
            number(chain, "q13_out") == 2
            and number(chain, "q14_out") == 16
            and number(chain, "q15_out") == 1
        ),
        "lc17_held_same_nonterminal_index2": (
            lc17_valid == 1
            and lc17_last == 0
            and lc17_same == 1
            and lc17_last_index == 2
        ),
        "lc17_blocked_only_at_pe7_inport0": (
            number(final_lc18, "lc17_bp") == 0x1FFFBFFFF
            and lc17_to_pe7_ready == 0
            and number(final_lc18, "lc18_bp") == 0x1FFFFFFFF
            and number(final_lc18, "pe7_in_bp") == 0x6
        ),
        "lc18_two_terminal_index3_outputs": (
            sum(
                1
                for row in lc18_edges
                if number(row, "lc18_port") == 0x630007
            )
            == 2
        ),
        "pe7_in0_held_after_index3_terminal": (
            number(final_lc18, "pe7_in_valid") == 0x1
            and number(final_lc18, "pe7_in_index") == 0x2
            and number(final_lc18, "pe7_in_bp") == 0x6
        ),
        "no_natural_terminal": (
            number(canonical, "slice_finish") == 0
            and number(dterm, "lc15_adv") == 1
        ),
    }
    if not (config_exact and mapping_exact and all(dynamic.values())):
        errors.append("LC17/LC18/PE7 keep-threshold chronology differs")
    ...
    report = {
        "schema": "node0004-v61-return-analysis-v1",
        "valid": not errors,
        "errors": errors,
        "RETURN_ANALYSIS": {
            ...
            "checks": {
                ...
                "simulator_argv_complete": argv_complete,
            },
        ...
        "HANG_ROOT_CAUSE": {
            ...
            "required": 3,
            "equation": (
                "IGA_PE_Inbuffer line 167 returns ready for a keep input only "
                "when buffer_last_bit && buffer_last_index <= keep_last_index"
            ),
            "counterexample": {
                "LC18_terminal_last_index": 3,
                "configured_keep_last_index": 2,
                "comparison": "3 > 2",
                "keep_ready": 0,
                "physical_LC17_to_LC18_ready": 1,
                "physical_LC17_to_PE7_in0_ready": 0,
                "physical_LC17_all_destination_ready": 0,
                "physical_LC17_advance_count": number(chain, "q15_out"),
            },
            "corrected_predicate": {
                "LC18_terminal_last_index": 3,
                "keep_last_index": 3,
                "comparison": "3 > 3 is false",
                "keep_ready": 1,
            },
            "why_source_id_is_not_changed": (
                "PE1 computes the flattened index LC15*8+LC9; LC15 is the "
                "correct keep operand and must be released after each LC9 "
                "eight-value loop."
            ),
            "rtl_proof": {
                "keep_predicate": {
                    "path": str(PE_INBUFFER_RTL),
                    "sha256": sha256_file(PE_INBUFFER_RTL),
                    "line": 167,
                },
                "all_destinations_ready": {
                    "path": str(LC_CONNECT_RTL),
                    "sha256": sha256_file(LC_CONNECT_RTL),
                    "line": 28,
                },
            },
            "final_json": {
            ...
            "mapping": {
                ...
                "logical_to_physical": {
                    "LC15": node_to_resource.get("DRAM_LC.LC15"),
                    "LC9": node_to_resource.get("DRAM_LC.LC9"),
                    "PE1": node_to_resource.get("LC_PE.PE1"),
                },
            },
        ...
        "QUALIFIED_COUNTERS": {
            ...
            "dterm_owner": dterm,
            "lc18_pe7_final": final_lc18,
            "lc18_pe7_edge_count": len(lc18_edges),
        },
        "BLOCKER_DELTA": {
            "closed": [
                "B_CONV_NODE0004_V60_OBSERVER_LOGICAL_PHYSICAL_LC_MAPPING",
                "B_CONV_NODE0004_V60_SIMULATOR_ARGV_FEATURE_RECEIPT",
                "B_CONV_NODE0004_LC17_TO_LC18_PE7_FIRST_DIVERGENCE_UNIQUE",
            ],
            "opened": [
        ...
        "RULE_CONFIRMATION": {
            ...
            "evidence": (
                "Qualified edge/ready records, exact mapper identity, final "
                "materialized config, and direct RTL consumer equation uniquely "
                "separate the changed config leaf from a DUT defect."
            ),
        },
