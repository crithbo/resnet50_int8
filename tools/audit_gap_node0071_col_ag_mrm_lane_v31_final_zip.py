...
SOURCE_ROOT = "r5_n71_gap_v30_arm_ready_factor_diag"
...
def configure() -> None:
    ...
    audit.EXTRA_VALIDATORS = [
        (
            ...
            ROOT / "tools/validate_gap_node0071_col_ag_mrm_lane_v31.py",
            [
                str(audit.ZIP),
                "--root-name",
                ROOT_NAME,
...
def main() -> int:
    ...
    signal_checks = {
        ...
        "feature_all_bound": all(
            signal.get("return", {}).get("col_ag_mrm_lane_binding", {}).values()
        ),
        "target_zip_exact": signal.get("target_zip_sha256") == sha256(audit.ZIP),
    }
    runner_checks = {
        "valid": runner.get("valid") is True,
        "positive_compile_reached": runner.get("positive_compile_reached") is True,
    ...
    hdl_checks = {
        ...
        "focused_compile_zero":
            positive.get("focused_xmr_sampler_compile", {}).get("exit_code") == 0,
    ...
    if not all(hdl_checks.values()):
        result["errors"].append("focused v31 package-local HDL gate differs")
    if result["errors"]:
    ...
    result.update(
        {
            "schema": "gap-node0071-v31-col-ag-mrm-final-audit-v1",
            "source_v30": source_receipt,
            ...
            "diagnostic_contract": {
                ...
                "last_proven_good": (
                    "two Buffer0 ARM full-row reads accepted with selected-bank "
                    "readiness high and NRM barrier low"
                ),
                "first_divergence": (
