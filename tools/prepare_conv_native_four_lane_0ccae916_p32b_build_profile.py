#!/usr/bin/env python3
"""Write the p32b same-epoch next-fresh build specification."""
...
BASE = "outputs/conv_native_four_lane_0ccae916_p32b_validowner"
BOUND = "outputs/conv_native_four_lane_0ccae916_p32b_validowner_source_bound"
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError("refusing to overwrite p32b build specification")
    builder = "tools/build_conv_native_four_lane_0ccae916_p32_validowner_package.py"
    family_validator = "tools/validate_conv_native_four_lane_0ccae916_p32_validowner_package.py"
    target_parser = "tools/conv_native_four_lane_p32_target_epoch_parser.py"
    runner_validator = "tools/validate_conv_native_four_lane_0ccae916_p32_runner_harness.py"
    generator = "tools/generate_server_source_bound_observer.py"
    generation = f"{BOUND}/source_bound_observer_generation.json"
    p31 = "artifacts/operator_config_validation/r5-server-test-packages/pending/r5_n4_0cc_p31_postclear.zip"
    analysis = "outputs/conv_native_four_lane_0ccae916_p31_return_analysis/report.json"
    ...
    validators = {
        ...
        "runner_control_flow": identity(runner_validator, "fixtures/server_package_pipeline_v1/cheap/core_identity_bootstrap.json"),
        "package_local_hdl": identity(generator, f"{BOUND}/generated/source_bound_causal_observer.svh"),
        "materialized_config": identity(builder, p31),
        ...
        "final_zip_content": identity(family_validator, p31),
    ...
    value = {
        ...
        "inputs": [
            record(p31, "package_identity"), record(builder, "package_identity"), record(analysis, "return_collector"),
            ...
            record(f"{BOUND}/generated/target_epoch_valid_owner_parser.py", "canonical_predicate"),
            record(f"{BOUND}/target_epoch_correlator_contract.json", "canonical_predicate"),
            record(target_parser, "parser"), record(post_helper, "return_core_contract"), record(prior, "storage"),
        ],
