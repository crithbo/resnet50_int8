#!/usr/bin/env python3
"""Create the exact final-ZIP release receipt for native-four-lane p24."""
...
BASE = ROOT / "outputs/conv_native_four_lane_0ccae916_p24_selport"
ZIP_PATH = BASE / "build" / f"{PACKAGE_ID}.zip"
BUILD = BASE / "build" / f"{PACKAGE_ID}.build.json"
FAMILY = BASE / "p24_family_audit_v2.json"
HARNESS = BASE / "p24_runtime_layout_harness_v2.json"
SHARED = BASE / "p24_shared_runtime_layout_v2.json"
PROFILE = BASE / "server_package_build_profile.json"
ANALYSIS = ROOT / "outputs/conv_native_four_lane_0ccae916_p23_return_analysis/report.json"
...
def main() -> int:
    ...
    if not all(path.is_file() for path in required):
        raise FinalizeError("required p24 release input is absent")
    ...
    scope = family["observer"]["focused_compile"]["p24_public_select_port"]
    runtime_ok = all(
    ...
    checks = {
        ...
        "formal_p23_analysis": analysis["valid"] is True and analysis["status"] == "P23_EPOCH_FLOW_PASS_CONNECT_SELECTION_SUCCESSOR_REQUIRED",
        ...
        "public_port_scope": scope["valid"] is True and scope["public_surface_only"] is True and scope["new_private_xmr"] is False and scope["positive"]["exit_code"] == 0 and scope["negative_leaf_deleted"]["exit_code"] != 0 and scope["negative_leaf_renamed"]["exit_code"] != 0 and scope["negative_wrong_sibling_path"]["exit_code"] != 0,
        "diagnostic_predicate_trace": scope["predicate_trace"]["valid"] is True and scope["predicate_trace"]["qualified_budget_consumed_by_state"] is False,
        "exact_logger_parser_trace": scope["logger_parser_trace"]["valid"] is True and scope["logger_parser_trace"]["normalization"] == "NONE",
    ...
    result = {
        "schema": "conv-native-four-lane-0ccae916-p24-final-zip-audit-v1",
        ...
        "audits": {
            "p23_return_analysis": receipt(ANALYSIS), "build_profile": receipt(PROFILE), "build": receipt(BUILD),
        ...
        "claim_boundary": "p24 is one c0 diagnostic. It does not claim c0 natural terminal, 27 natural terminals, formal 320D, E3/E4/E5, numeric correctness or performance before formal server return.",
        "server_action": False,
    }
    if OUTPUT.exists():
        raise FinalizeError("refusing to overwrite p24 final audit")
