...
import tools.validate_node0004_v29_runner_controls as prior  # noqa: E402
...
def write_stubs(stub_root: Path, python: Path) -> None:
    ...
    anchor = (
        ...
        "feature=RETURN_OBS_DATAHUB_DRAIN enabled=1 "
        "limit_name=RETURN_OBS_DATAHUB_DRAIN_LIMIT limit=64\n"
    )
    addition = anchor + (
        "0 | DIAGNOSTIC_FEATURE_ENABLE_V1 | "
        "feature=RETURN_OBS_MSE4_DESCRIPTOR enabled=1 "
        "limit_name=RETURN_OBS_MSE4_DESCRIPTOR_LIMIT limit=96\n"
    )
    if text.count(anchor) != 1:
        raise ValueError("safe-stub marker anchor differs")
...
def main() -> int:
    ...
    report = base.validate(
        args.zip.resolve(), args.sidecar.resolve(), args.bash.resolve(),
        args.python.resolve(), expected_zip_sha256=args.expected_zip_sha256,
        report_schema="node0004-v30-runner-controls-v1",
