#!/usr/bin/env python3
"""Build the v3 one-command stock-RTL package for the authorized human MAC JSON."""
...
NATIVE = BASE / "nativechain_v3_a/model_execplan/output/mac_int32_uint8_graph"
DATA = BASE / "v3_release/generated_data"
INSTALL = "human_mac_int32_uint8_v3_stock_rtl_fd2"
...
set -u
if [ "$#" -ne 1 ]; then echo "Usage: bash PREPARE_AND_RUN.sh /absolute/path/to/NDP_copy03" >&2; exit 2; fi
case "$1" in /*) ;; *) echo "NDP_copy path must be absolute" >&2; exit 2;; esac
package_root="$(cd "$(dirname "${{BASH_SOURCE[0]}}")" && pwd)"
ndp_root="$(cd "$1" && pwd)"
if [ "$(basename "$ndp_root")" != "NDP_copy03" ]; then echo "Authorized server root is NDP_copy03 only" >&2; exit 2; fi
name="{INSTALL}"
...
def build() -> None:
    ...
    shutil.copy2(BASE / "mac_int32_uint8.corrected_v3.json", OUT / "provenance/mac_int32_uint8.corrected_v3.json")
    ...
    write(OUT / "README.md", f"# Human MAC stock-RTL test v3\n\nRun only after obtaining the group-C lease:\n\n`bash PREPARE_AND_RUN.sh /absolute/path/to/NDP_copy03`\n\nExpected return: `{INSTALL}_return.zip` and `.sha256` sidecar.\n")
    manifest = {
        "schema": "human-authored-mac-int32-uint8-stock-rtl-package-v2",
        ...
        "parent_candidate": {"path": "provenance/mac_int32_uint8.corrected_v2.json", "size_bytes": 13942, "sha256": "24002ec87abd2e1c5f659003c61aa6176d2d7bd18dbfebeae890e11d80b36eb6"},
        "corrected_candidate": {"path": "provenance/mac_int32_uint8.corrected_v3.json", "size_bytes": 13941, "sha256": "b7087828cf6235147d1c61a40835ed4a38ed437abc14ad6871708ad487becc26"},
        "authorized_delta": {"field": "general_array.outport.src_id", "from": 1, "to": 0},
        "lc2_last_index": 1,
        "server_root": "NDP_copy03",
        "server_lease_required": True,
        "server_lease_present_at_build": False,
        "only_command": "bash PREPARE_AND_RUN.sh /absolute/path/to/NDP_copy03",
