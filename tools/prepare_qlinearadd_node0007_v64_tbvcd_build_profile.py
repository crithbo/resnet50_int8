#!/usr/bin/env python3
"""Compile the current build-gate registry profile for exact QAdd v64."""
...
def main() -> int:
    family_tree = OUT / "gates/failure_delta_tree.json"
    family_zip = OUT / "gates/failure_delta_zip.json"
    shared_vcd = GATE / "vcd_tree.json"
    ...
    aliases = {
        "hdl.json": family_zip,
        "source_bound.json": family_zip,
        "vcd_exact_zip.json": family_zip,
    }
    for name, source in aliases.items():
        value = json.loads(source.read_text(encoding="utf-8"))
        value["alias_surface"] = name.removesuffix(".json")
        value["source_receipt"] = receipt(source, "family_failure_delta")
        write(GATE / name, value)

    required = [
        family_tree, family_zip, GATE / "runner_zip.json", GATE / "nativeflow.json",
        GATE / "hdl.json", GATE / "lexical_zip.json", GATE / "postsim.json",
        shared_vcd, GATE / "runtime_layout.json", FIRST / "validation.json",
    ]
    ...
    for gate_id, source in cheap_sources.items():
        value = {
            ...
            "errors": [],
            "warnings": [],
            "source_receipt": receipt(source, gate_id),
        }
    ...
    inputs = [
        ...
        receipt(ROOT / "tools/build_qlinearadd_node0007_v64_tbvcd_failure_fix.py", "package_identity"),
        ...
        receipt(TREE / "diagnostics/tb_vcd_exact_dump_plan.json", "probe_plan"),
        receipt(TREE / "provenance/PACKAGE_BUILD_FAILURE_RULE_AUDIT.json", "package_local_hdl"),
    ...
    family_validator = ROOT / "tools/validate_qlinearadd_node0007_v64_tbvcd_failure_delta.py"
    validators = {
        "core_identity_bootstrap": identity(ROOT / "tools/build_qlinearadd_node0007_v64_tbvcd_failure_fix.py", OUT / "build/build_receipt.json"),
        ...
        "package_local_hdl": identity(family_validator, GATE / "hdl.json"),
        ...
        "tb_vcd_bounded_causal_cone_final_zip": identity(family_validator, GATE / "vcd_exact_zip.json"),
        "first_fresh_extra_audit": identity(ROOT / "tools/validate_server_first_fresh_extra_audit.py", FIRST / "validation.json"),
        "final_zip_content": identity(ROOT / "tools/build_qlinearadd_node0007_v64_tbvcd_failure_fix.py", OUT / "build/build_receipt.json"),
    ...
    spec = {
        ...
        "rule_change_epoch": {"epoch_id": "tb-vcd-bounded-causal-cone-optional-v1-0820e1733437+qadd-failure-delta-v1", "first_fresh_after_change": True, "prior_audit_receipt": None},
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "tools/server_package_pipeline.py"), "prepare", "--spec", str(spec_path), "--registry", str(ROOT / "contracts/server_package_build_gate_registry_v1.json"), "--workspace-root", str(ROOT), "--output", str(profile_path)],
        cwd=ROOT,
        capture_output=True,
        text=True,
        timeout=60,
    ...
    if profile.get("contract_valid") is not True or profile.get("preflight", {}).get("pass") is not True:
        raise RuntimeError(json.dumps(profile, indent=2))
