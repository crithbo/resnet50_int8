#!/usr/bin/env python3
"""Exercise p41 compile-core evidence while enforcing mandatory VPD controls."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="p41_compile_core_") as temporary:
        root = Path(temporary)
        ...
        checks = {
            ...
            "mandatory_waveform_controls": all(
                token in text for token in ("DUMP_VCD=1", "DUMP_FSDB=0", "TB_DUMP_FSDB=0")
            ) and "DUMP_VCD=0" not in text,
            "bounded_tokens": all(
                token in text
                for token in ("compile_log_head.txt", "compile_log_tail.txt", "compile_first_error.txt")
            ),
            "bootstrap_publisher": '--bootstrap-root "$bootstrap_root"' in text,
            "waveform_collector_before_post_sim": text.find('"$waveform_helper" collect-runtime')
            < text.find('python3 "$post_sim_helper" finalize --request "$post_sim_request"'),
        }
        ...
        checks.update(
            {
                ...
                "first_error_actual": "actual first compile failure"
                in (bootstrap / "compile_first_error.txt").read_text(encoding="utf-8"),
        ...
        publisher.write_text(
            publisher.read_text(encoding="utf-8").replace(
                'FIXED_RESULT_ROOT = "/home/panqs/ndp/simresult"', f"FIXED_RESULT_ROOT = {str(fixed)!r}"
            ),
            encoding="utf-8",
            newline="\n",
        )
        ...
        checks["compile_fail_return_published"] = (
            publication.returncode == 0 and target.is_file() and Path(str(target) + ".sha256").is_file()
        )
        returned: list[str] = []
        if target.is_file():
            ...
            checks["compile_core_complete"] = (
                manifest.get("compile_core_complete") is True and manifest.get("runner_exit_code") == 2
            )
            checks["compile_not_started_waveform_absent"] = all(
                not name.lower().endswith((".vpd", ".vcd", ".fsdb"))
                and "/waveforms/" not in name.lower()
                for name in returned
            )
        else:
            checks.update(
                {
                    "return_exact_allowlist": False,
                    "compile_core_complete": False,
                    "compile_not_started_waveform_absent": False,
                }
            )
        details.update(
            {
                "checks": checks,
                "returned_members": returned,
                "publication": {
                    "exit_code": publication.returncode,
    ...
    report = {
        "schema": "conv-native-four-lane-p41-compile-core-harness-v1",
        "valid": not errors,
        "pass": not errors,
        "errors": errors,
        "scenarios": {
            "compile_fail": {
                ...
                "fixed_result_return_published": not errors,
                "compile_core_complete": not errors,
