#!/usr/bin/env python3
"""Independent final-ZIP audit for the node0071 -> node0075 v5 package."""
...
def frozen_v3_payload_receipt(
    ...
    allowed_namespace_changes = {
        "workload/sca_cfg.json",
        "workload/sca_cfg_D.json",
    }
    unexpected_changed = sorted(set(changed) - allowed_namespace_changes)
    namespace_normalized = {}
    for path in sorted(set(changed) & allowed_namespace_changes):
        normalized = v4_entries[path].replace(
            NAME.encode("utf-8"),
            b"r5_n71_n75_e1f_native_v3",
        )
        namespace_normalized[path] = {
            "normalized_byte_equal": normalized == v3_entries[path],
            "v3_sha256": sha256_bytes(v3_entries[path]),
            "v5_sha256": sha256_bytes(v4_entries[path]),
            "normalization": (
                f"{NAME} -> r5_n71_n75_e1f_native_v3"
            ),
        }
    normalized_valid = all(
        item["normalized_byte_equal"]
        for item in namespace_normalized.values()
    )
    valid = (
        not missing
        and not unexpected_changed
        and set(changed) == allowed_namespace_changes
    ...
    return {
        ...
        "namespace_only_sca_changes": namespace_normalized,
        "workload_member_count": sum(
            path.startswith("workload/") for path in frozen_paths
        ),
        "exact_byte_equal_excluding_namespace_sca":
            not missing and not unexpected_changed,
        "causal_materialized_semantics_unchanged": valid,
...
def validate(zip_path: Path, sidecar: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "v3_frozen_causal_payload_receipt": frozen_payload["valid"],
    ...
    release_gate_matrix = {
        "core_always": {
            ...
            "applicable": True,
            "reason": "fresh v5 identity and final ZIP",
        ...
        "package_local_hdl": {
            "gate_id":
                "CDA-SERVER-PACKAGE-LOCAL-OBSERVER-HDL-SYNTAX-SCOPE-POSITIVE-001",
            "applicable": True,
            "reason": "v5 changes package-local observer clock/reset consumers",
            "changed_surface": True,
            "evidence": {
                "hdl_gate": hdl["pass"],
        ...
        "materialized_config": {
            ...
            "applicable": True,
            "reason": (
                "v5 changes only the two SCA package-namespace path strings; "
                "final config/mapping/bitstream/execplan/golden and the causal "
                "transaction slice are frozen"
            ),
            "changed_surface": "PACKAGE_NAMESPACE_ONLY_NO_CAUSAL_CHANGE",
            "evidence": {
                ...
                "causal_transaction_ledger": {
                    ...
                    "receipt_reuse": True,
                    "reason": (
                        "final config/mapping/bitstream/execplan/golden and the "
                        "causal slice are byte-equal to frozen v3; the two SCA "
                        "members differ only by package namespace and normalize "
                        "exactly to v3"
                    ),
                },
                "boundary_microtrace": {
                    ...
                    "receipt_reuse": True,
                    "reason": (
                        "no threshold/mask/pingpong/keep/buffer-valid/capacity/"
                        "lifetime/CONFIG predicate changed"
                    ),
                },
            },
            "blocking": True,
    ...
    return {
        "schema":
            "node0071-node0075-native-ordering-v5-final-zip-self-audit-v1",
