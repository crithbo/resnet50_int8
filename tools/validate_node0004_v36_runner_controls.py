...
import tools.validate_node0004_v34_runner_controls as prior  # noqa: E402
...
def write_stubs(stub_root: Path, python: Path) -> None:
    ...
    addition = anchor + (
        "0 | DIAGNOSTIC_FEATURE_ENABLE_V1 | "
        "feature=RETURN_OBS_B5RD enabled=1 "
        "limit_name=RETURN_OBS_B5RD_LIMIT limit=96\n"
    )
    if text.count(anchor) != 1:
        raise ValueError("safe-stub v36 marker anchor differs")
    make.write_text(
...
def main() -> int:
    ...
    report = base.validate(
        ...
        report_schema="node0004-v36-runner-controls-v1",
