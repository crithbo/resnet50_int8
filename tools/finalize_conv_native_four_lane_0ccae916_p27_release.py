#!/usr/bin/env python3
"""Create the exact final-ZIP release receipt for native-four-lane p27."""
...
BASE = ROOT / "outputs/conv_native_four_lane_0ccae916_p27_b5release"
...
FAMILY = BASE / "p27_family_audit.json"
HARNESS = BASE / "p27_runtime_layout_harness.json"
SHARED = BASE / "p27_shared_runtime_layout_from_harness.json"
...
def main() -> int:
    ...
    if not all(path.is_file() for path in required):
        raise FinalizeError("required p27 release input is absent")
    ...
    result = {
        "schema": "conv-native-four-lane-0ccae916-p27-final-zip-audit-v1",
        ...
        "claim_boundary": (
            "p27 is one c0 generated source-bound diagnostic for Buffer5 last-write/read-response timing. It does not "
            "claim c0 natural terminal, 27 natural terminals, formal 320D, E3/E4/E5, numeric correctness or performance."
        ),
        "server_action": False,
    }
    if OUTPUT.exists():
        raise FinalizeError("refusing to overwrite p27 final audit")
    OUTPUT.write_text(
