...
def inject_post_receipt_preflight_failure(package: Path) -> None:
    ...
    new = (
        ...
        "    status = base.main()\n"
        "    if (\n"
        "        os.environ.get('HARNESS_FAIL_AFTER_PREFLIGHT_RECEIPT') == '1'\n"
        "        and len(sys.argv) > 1\n"
    ...
    if runtime.count(old) != 1:
        raise ValueError("runtime main anchor differs")
    runtime_path.write_text(
        runtime.replace(old, new, 1),
        encoding="utf-8",
        newline="\n",
    )
    refresh_manifest(package)
...
def run_case(
    ...
    *,
    ...
    if preflight_mutation:
        inject_post_receipt_preflight_failure(package)
    stub = root / "stub"
    ...
    creatable_absent_before = (
        not (server / "install/cfg_pkg").exists()
        and not (server / "install/codex_runs").exists()
    )
    if missing_parent:
    ...
    env["HARNESS_FAIL_AFTER_PREFLIGHT_RECEIPT"] = (
        "1" if preflight_mutation else "0"
    )
    ...
    return {
        ...
        "preflight_mutation": preflight_mutation,
        ...
        "preexisting_install_verified": missing_parent is None,
        "creatable_parents_initially_absent": creatable_absent_before,
        "creatable_parents_real_after": (
            (server / "install/cfg_pkg").is_dir()
            and not (server / "install/cfg_pkg").is_symlink()
            and (server / "install/codex_runs").is_dir()
            and not (server / "install/codex_runs").is_symlink()
        ),
...
def main() -> int:
    ...
    checks = {
        ...
        "positive_layout_and_publication": all(
            ...
            and row["preexisting_install_verified"]
            and row["creatable_parents_initially_absent"]
            and row["creatable_parents_real_after"]
            and not row["unknown_items_deleted_or_overwritten"]
