...
ZIP_SHA256 = "5094fc3e01a04c1931b81c4db3a67bf2f6b82f424124d0311866d03004997c90"
...
def feature_binding_controls(package: Path, root: Path) -> dict:
    ...
    argv_tokens = ["simv"] + [
        token
        for _, enable, limit in FEATURES
        for token in (enable, limit)
    ] + [
    ...
    limit_negative = run_case(
        [
            token
            for token in argv_tokens
            if token != "+RETURN_OBS_LC18_PE7_LIMIT=96"
        ],
        markers,
    )
    marker_negative = run_case(
        ...
        [
            row
            for row in markers
            if "feature=RETURN_OBS_LC18_PE7 " not in row
        ],
    )
...
def main() -> int:
    ...
    parser.add_argument("--source-v32", required=True, type=Path)
    ...
    source_path = args.source_v32.resolve()
    ...
    binding = json.loads(
        entries.get(
            "provenance/current_local_rtl_binding.json", b"{}"
        )
    )
    with tempfile.TemporaryDirectory(prefix="v33-final-audit-") as temp:
        root = Path(temp)
    ...
    checks = {
        ...
        "only_expected_source_changes": (
            set(changed) == expected_changed and not added
        ),
        "current_rule_receipts": all(
            value in receipt_text for value in CURRENT_RECEIPTS.values()
        ),
        "observer_narrow_feature": (
            observer.count("LC18_PE7_BOUNDARY_V1") == 1
            and observer.count(
                "feature=RETURN_OBS_LC18_PE7 enabled=%0d"
            )
            == 1
            and "return_obs_write_lc18_pe7_state(event_name);" in observer
            and "LC18_PE7_EDGE_V1" in observer
        ),
        "actual_runner_binding": (
            prepare.count("+RETURN_OBS_LC18_PE7") == 4
            and prepare.count("+RETURN_OBS_LC18_PE7_LIMIT=96") == 2
            and "+define+NATIVE_RETURN_OBSERVER_ENABLE" in prepare
            and "+incdir+$package_root/tb_probe" in prepare
        ),
        "collector_binding": (
            runtime.count('"feature": "RETURN_OBS_LC18_PE7"') == 1
            and runtime.count(
        ...
        "physical_mapping_binding": mapping
        == {
            "DRAM_LC.LC15": "LC17",
            "DRAM_LC.LC9": "LC18",
            "LC_PE.PE1": "PE7",
        ...
        "feature_binding_positive": (
            ...
            and len(feature_controls["positive"].get("features", [])) == 9
        ),
    ...
    report = {
        "schema": "node0004-v33-final-zip-rule-self-audit-v1",
        "valid": not errors,
        ...
        "source_v32_sha256": prior.sha256_file(source_path),
        ...
        "runner_controls_sha256": prior.sha256_file(
            args.runner_controls
        ),
