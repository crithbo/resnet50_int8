#!/usr/bin/env python3
"""Write p35 first-fresh shadow build specification."""
...
BASE = "outputs/conv_native_four_lane_0ccae916_p35_armknown"
BOUND = "outputs/conv_native_four_lane_0ccae916_p35_armknown_source_bound"
OUTPUT = ROOT / BASE / "server_package_build_spec_v2.json"
EPOCH = "20260811-native-live-causal-partial-exit-v1"
BUILDER = "tools/build_conv_native_four_lane_0ccae916_p35_armknown_package.py"
GENERATION_REPORT = f"{BOUND}/source_bound_observer_generation.json"
CHEAP_REPORT = f"{BOUND}/source_bound_observer_cheap_check.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError("refusing to overwrite p35 build specification")
    builder = BUILDER
    generator = "tools/generate_server_source_bound_observer.py"
    generation = GENERATION_REPORT
    source = "artifacts/operator_config_validation/r5-server-test-packages/pending/r5_n4_0cc_p34b_armtoken.zip"
    analysis = "outputs/conv_native_four_lane_0ccae916_p34b_return_analysis/report.json"
    ...
    validators = {
        ...
        "runner_control_flow": identity("tools/validate_conv_native_four_lane_0ccae916_p34_runner_harness.py", fixture),
        ...
        "diagnostic_semantics": identity(parser, f"{BOUND}/source_bound_probe_plan.json"),
    ...
    value = {
        ...
        "rule_change_epoch": {"epoch_id": EPOCH, "first_fresh_after_change": True, "notification_acknowledged": True, "rule_ids": ["CDA-SERVER-DIAGNOSTIC-PARTIAL-EXIT-LIVE-CAUSAL-RECORD-001"]},
        ...
        "cheap_check_reports": [
            ...
            {"gate_id": "source_bound_observer_generation", "path": CHEAP_REPORT, "sha256": sha(ROOT / CHEAP_REPORT)},
