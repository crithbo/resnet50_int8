...
ZIP = (
    ...
    / "r5_n71_gap_v9_ingress_rule.zip"
)
SOURCE_V8_ZIP = (
    ROOT
    / "artifacts/operator_config_validation/r5-server-test-packages"
...
OUTPUT = (
    ROOT
    / "artifacts/operator_config_validation/r5-server-test-packages"
    / "r5_n71_gap_v9_ingress_rule.final_zip_rule_self_audit.json"
)
ROOT_NAME = "r5_n71_gap_v9_ingress_rule"
RULES = {
...
def audit() -> dict[str, Any]:
    ...
    dual_receipt, dual_stdout = run_command(
        "final_zip_dual_ingress_validator_and_negative_controls",
        [
            sys.executable,
            str(
                ROOT
                / "tools/validate_gap_node0071_v8_dual_ingress.py"
            ),
            str(ZIP),
        ],
        ROOT,
    )
    command_receipts.append(dual_receipt)
    dual_report = json.loads(dual_stdout)
    refresh_receipt, refresh_stdout = run_command(
        "final_zip_v8_to_v9_rule_refresh_validator_and_negative_controls",
        [
            sys.executable,
            str(
                ROOT
                / "tools/validate_gap_node0071_v9_rule_refresh.py"
            ),
            str(SOURCE_V8_ZIP),
            str(ZIP),
        ],
    ...
    command_receipts.append(refresh_receipt)
    refresh_report = json.loads(refresh_stdout)

    with tempfile.TemporaryDirectory(
        prefix="gap-node0071-v9-final-audit-"
    ) as temporary:
    ...
    dual_controls = dual_report.get("negative_controls", {})
    dual_controls_pass = all(
        value.get("failed_closed") is True
        for value in dual_controls.values()
    )
    refresh_controls = refresh_report.get("negative_controls", {})
    refresh_controls_pass = all(
        value.get("failed_closed") is True
        for value in refresh_controls.values()
    )
    ...
    if not (
        ...
        and dual_controls_pass
        and refresh_controls_pass
    ...
    if (
        ...
        or dual_report.get("status") != "PASS"
        or refresh_report.get("status") != "PASS"
    ...
    rule_checks = {
        "complete_rebuild_or_frozen_reuse": {
            ...
            "reason":
                ...
                "reuse; only identity/SCA namespace/manifest/README/rule "
        ...
        "dual_ingress_qualified_localization": {
            "status": (
                "PASS"
                if dual_report.get("status") == "PASS"
                else "FAIL"
            ),
            "claim_boundary":
                "observer-only qualified counters; not functional progress",
        },
        "v8_to_v9_rule_refresh_boundary": {
            "status": (
                "PASS"
            ...
            "claim_boundary":
                "only five authorized identity/namespace/receipt files "
                "changed; 73 numeric and 120 immutable files are exact",
        },
    ...
    return {
        ...
        "negative_control_exit_codes": {
            ...
            "dual_ingress_validator_harness":
                dual_receipt["exit_code"],
            "v8_to_v9_rule_refresh_validator_harness":
                refresh_receipt["exit_code"],
        ...
        "negative_controls": {
            ...
            "dual_ingress": dual_controls,
            "v8_to_v9_rule_refresh": refresh_controls,
        ...
        "validator_report_sha256": {
            ...
            "dual_ingress":
                dual_receipt["stdout_sha256"],
            "v8_to_v9_rule_refresh":
                refresh_receipt["stdout_sha256"],
            "package_preflight":
