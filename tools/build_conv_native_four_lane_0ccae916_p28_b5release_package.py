#!/usr/bin/env python3
"""Build p28 with an instance-order-stable source-bound Buffer5 observer."""
...
import build_conv_native_four_lane_0ccae916_p27_b5release_package as previous
...
DEFAULT_OUTPUT = ROOT / "outputs/conv_native_four_lane_0ccae916_p28_b5release/build"
...
def install_generated(package: Path) -> dict[str, dict[str, Any]]:
    ...
    contract = {
        ...
        "claim_boundary": (
            "p28 source-bound Buffer5 c0 diagnostic only; production compile/simulation, natural terminal, "
...
def patch_runner(package: Path) -> dict[str, Any]:
    ...
    if any(token not in text for token in required):
        raise BuildError("inherited runner source-bound four-way binding is incomplete")
    return {
...
def patch_contract(package: Path) -> dict[str, Any]:
    ...
    value["claim_boundary"] = (
        "p28 mechanically preserves the p27 installed payload/config and replaces only the generated decision "
        "metrics with instance-order-stable sticky class_seen semantics under install-only V2."
    )
...
def patch_pointer_readme(package: Path) -> None:
    ...
    value.update(
        {
            "schema": "conv-native-four-lane-p28-source-bound-pointer-v1",
    ...
    (package / "README.md").write_text(
        "# Native four-lane Conv p28 source-bound Buffer5 release diagnostic\n\n"
        "Fresh instance-order-stable successor of held p27. The 87 installed payload members remain byte-identical. "
...
def patch_manifest(
    ...
    if base.sha256(legacy) != SOURCE_LEGACY_OBSERVER_SHA256:
        raise BuildError("exact inherited legacy observer identity differs")
    value.update(
        {
            "schema": "conv-native-four-lane-0ccae916-p28-source-bound-package-v1",
            "package_identity": PACKAGE_ID,
    ...
    value["delivery_successor"] = {
        ...
        "reason": "p27 count_nonzero decisions were instance-order-sensitive; p28 keeps the same causal cone with sticky class_seen decisions",
    ...
    for item in additions:
        if item["target_path"] in existing_targets:
            existing = next(
                ...
                if isinstance(candidate, dict) and candidate.get("target_path") == item["target_path"]
            )
            if existing != item:
                raise BuildError("inherited source-bound return member differs")
        else:
    ...
    value["release_gate_matrix"].update(
        {
            ...
            "materialized_config": {
                ...
                "blocking": False,
                "pass": True,
                "scope": "87 p27 installed payload members byte-equal and SCA identity-normalized equal",
...
def main() -> int:
    ...
    if any(target.exists() for target in targets):
        raise BuildError("refusing to overwrite p28 output")
    if SOURCE_ZIP.stat().st_size != SOURCE_BYTES or base.sha256(SOURCE_ZIP) != SOURCE_SHA256:
        raise BuildError("exact p27 source differs")
    ...
    if (
        ...
        raise BuildError("frozen p27 payload differs")
    zip_path = output / f"{PACKAGE_ID}.zip"
    base.deterministic_zip(package, zip_path)
    with tempfile.TemporaryDirectory(prefix=".p28_repeat_", dir=ROOT) as temporary:
    ...
    if not deterministic:
        raise BuildError("p28 deterministic double build differs")
    zip_sha = base.sha256(zip_path)
    Path(str(zip_path) + ".sha256").write_text(
        f"{zip_sha}  {zip_path.name}\n", encoding="ascii", newline="\n"
    )
    report = {
        "schema": "conv-native-four-lane-p28-source-bound-build-v1",
        "status": "PACKAGE_BUILT_PENDING_FINAL_AUDIT",
        "package_identity": PACKAGE_ID,
        "source_p27_zip_sha256": SOURCE_SHA256,
