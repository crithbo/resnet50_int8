...
from tools.node0004_hang_localization_runtime_v7 import (  # noqa: E402
    package_records,
    preflight,
    sha256,
)
...
INSTALL_NAME = "r5_n4_hw_v8_hangloc_fourway"
SOURCE_INSTALL_NAME = "r5_n4_hw_v7_hangloc_bind"
SOURCE_ZIP = (
    ROOT
...
RETURN_ZIP_SHA256 = (
    "787f68073da9d283f7862b1eb44c086c3158e207a1e082cee5f736d29a5ad606"
)
PLAN_SHA256 = (
    "fdd8cf3e14128d40a141655f60cd79b80d46c6f6626446c56daba837ee9cc8a6"
)
SERVER_RULE_SHA256 = (
...
RUNTIME_SOURCE = ROOT / "tools/node0004_hang_localization_runtime_v7.py"
...
def _extract_c0_only(destination: Path) -> Path:
    if sha256(SOURCE_ZIP) != SOURCE_ZIP_SHA256:
        raise BuildError("frozen v7 source ZIP identity differs")
    ...
    with zipfile.ZipFile(SOURCE_ZIP) as archive:
        if archive.testzip() is not None:
            raise BuildError("frozen v7 source ZIP CRC failed")
...
def _readme() -> str:
    return f"""# node0004 v8 four-way observer-binding package

Classification: `DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX`.

This package is not a Conv result candidate and cannot establish E4/E5. It
supersedes v7 by adding the manifest-level source/size/SHA, compile include,
compile enable, runtime, time-zero receipt, return allowlist, and signal-trap
contract required by `CDA-SERVER-OBSERVER-BINDING-FOUR-WAY-001`. It preserves
the frozen v7 c0 workload and stops on either natural c0 terminal, four
consecutive 262144-cycle no-progress windows, or an 8388608-cycle diagnostic
budget. It does not modify functional RTL or the server TB.
...
def build_directory(destination: Path) -> tuple[Path, dict[str, Any]]:
    ...
    manifest = {
        "schema": "resnet50-node0004-hang-localization-package-v8",
        "install_name": INSTALL_NAME,
        "classification": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX",
        "status": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX_READY_NOT_RUN",
        "candidate_release": False,
        "evidence_level": "E2_LOCAL_PLUS_V7_FOUR_WAY_MANIFEST_FAILURE",
        "run_ids": ["c0"],
        "frozen_source_package": {
            "path": SOURCE_ZIP.relative_to(ROOT).as_posix(),
            "sha256": SOURCE_ZIP_SHA256,
        },
        "bound_v6_return_sha256": RETURN_ZIP_SHA256,
        "active_receipts": {
        ...
        "observer_binding_four_way": {
            ...
            "compile": {
                "package_local_incdir": "tb_probe",
                "enable_macro": "+define+NATIVE_RETURN_OBSERVER_ENABLE",
                "actual_compile_argv_receipt": (
            ...
            "runtime_return": {
                ...
                "time_zero_enabled_marker": "[RETURN_OBSERVER] enabled",
                "return_allowlist_paths": [
                ...
                "signal_traps": ["HUP", "INT", "TERM"],
            },
        },
        "package_side_repair": {
            "classification": "PACKAGE_OBSERVER_BINDING_INCOMPLETE",
            "first_divergence": (
                "v7 final manifest omitted the explicit four-way observer "
                "source size and runtime/return binding contract"
            ),
            "v6_observer_bytes": 0,
            "v6_progress_window_count": 0,
            "v6_run_exit_status": 125,
            "v6_signal_status": "INT",
            "repair": (
                "retain v7 compile macro/include repair and add the exact "
                "manifest-level four-way binding contract"
            ),
            "functional_semantics_changed": False,
        },
        "unresolved_boundary": (
            "c0 Start_Comp accepted; after repairing observer compile binding, "
            "the same eight qualified producer-to-consumer boundaries remain "
            "the only diagnostic decision table"
        ),
        "progress_contract": {
        ...
        "server_action": False,
        "superseded_diagnostic_package": {
            "name": "r5_n4_hw_v7_hangloc_bind.zip",
            "sha256": SOURCE_ZIP_SHA256,
            "status": "PACKAGE_OBSERVER_BINDING_INCOMPLETE",
        },
...
def _repeat(package: Path, zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="node0004-v8-repeat-") as temporary:
...
def main() -> int:
    ...
    try:
        ...
        receipt = {
            "schema": "node0004-hang-localization-package-validation-v8",
            "status": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX_READY_NOT_RUN",
            "package": str(package),
            "zip": str(zip_path),
