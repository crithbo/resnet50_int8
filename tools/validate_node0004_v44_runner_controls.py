...
import tools.validate_node0004_v38_runner_controls as prior
...
base.FEATURES = {
    **base.FEATURES,
    "RETURN_OBS_LC9_SPLIT": (
        "+RETURN_OBS_LC9_SPLIT",
        "+RETURN_OBS_LC9_SPLIT_LIMIT=128",
    ),
}
...
def write_stubs(stub_root: Path, python: Path) -> None:
    ...
    if text.count(anchor) != 1:
        raise ValueError("safe-stub WRTERM marker anchor differs")
    make.write_text(
...
def main() -> int:
    ...
    report = base.validate(
        ...
        report_schema="node0004-v44-runner-controls-v1",
