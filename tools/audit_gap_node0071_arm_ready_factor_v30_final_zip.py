...
SOURCE_ROOT = "r5_n71_gap_v29_mse0_buffer_prep_group0_diag"
...
def configure() -> None:
    ...
    manifest = json.loads(
        (PACKAGE_DIR / ROOT_NAME / "TEST_PACKAGE_MANIFEST.json").read_text(
            encoding="utf-8"
        )
    )
    audit.RULES = {
        item["path"]: sha256(ROOT / item["path"])
        for item in manifest["final_zip_rule_self_audit_contract"]["read_receipt"]
    }
    ...
    audit.EXTRA_VALIDATORS = [
        (
            "arm_ready_factor_validator_and_controls",
            ROOT / "tools/validate_gap_node0071_arm_ready_factor_v30.py",
            [
                str(audit.ZIP),
                "--root-name",
                ROOT_NAME,
...
def main() -> int:
    ...
    signal_checks = {
        ...
        "all_signal_scope_true": all(
            value is True or value == "TERM"
            for value in signal.get("current_rule_scope", {}).values()
        ),
        "arm_ready_feature_all_bound": all(
            signal.get("return", {})
            .get("buffer0_arm_ready_factor_binding", {})
            .values()
        ),
        "target_zip_exact": signal.get("target_zip_sha256") == sha256(audit.ZIP),
    }
    runner_checks = {
        ...
        "target_zip_exact": runner.get("target_zip_sha256") == sha256(audit.ZIP),
    }
    positive = hdl.get("positive", {})
    hdl_checks = {
        "status_pass": hdl.get("status") == "PASS",
        "pass": hdl.get("pass") is True,
        "target_zip_exact":
            hdl.get("target_receipt", {}).get("zip_sha256") == sha256(audit.ZIP),
        "observer_exact":
        ...
        "local_projection_compile_zero":
            positive.get("local_projection_compile", {}).get("exit_code") == 0,
        "focused_xmr_compile_zero":
            positive.get("focused_xmr_compile", {}).get("exit_code") == 0,
        "scoped_identifier_closure":
            positive.get("scoped_identifier_closure", {}).get("valid") is True,
        "all_negatives_fail_closed":
    ...
    if not all(hdl_checks.values()):
        result["errors"].append("focused v30 package-local HDL gate differs")
    if result["errors"]:
    ...
    result.update(
        {
            "schema": "gap-node0071-v30-buffer0-arm-ready-factor-final-audit-v1",
            "source_v29": source_receipt,
            ...
            "diagnostic_contract": {
                ...
                "last_proven_good": (
                    "8 active-window Buffer0 accepts reach prepared writes; "
                    "2 ARM reads clear and 5 prepared reads produce data_vld"
                ),
                "first_divergence": (
                    "BUFFER0_ARM_READ_REQUEST_0xFF_HELD_WITH_"
                    "BUF2ARM_REQ_READY_0_AFTER_TWO_ACCEPTS"
                ),
                "v29_observer_defect_corrected": True,
                ...
                "active_rtl_assumed_to_fix_gap": False,
            },
            "package_release": {
