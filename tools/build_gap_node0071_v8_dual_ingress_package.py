...
INSTALL_NAME = "r5_n71_gap_v8_dual_ingress"
SOURCE_NAME = "r5_n71_gap_v7_finalaudit"
SOURCE_ZIP = (
    ROOT
...
SOURCE_SHA256 = (
    "6ae39b218e622f9937753dd4d4d649b1d2a7420c49ec5ed71d00fe8c26abd068"
)
OBSERVER_SOURCE = ROOT / "tools/gap_node0071_v8_dual_ingress_observer.svh"
OUTPUT_ROOT = (
    ROOT
    / "artifacts/operator_config_validation/r5-server-test-packages"
)
...
def extract_source(destination: Path) -> Path:
    if sha256(SOURCE_ZIP) != SOURCE_SHA256:
        raise BuildError("frozen v7 source differs")
    package = destination / INSTALL_NAME
    package.mkdir(parents=True, exist_ok=False)
    with zipfile.ZipFile(SOURCE_ZIP) as archive:
        if archive.testzip() is not None:
            raise BuildError("frozen v7 source CRC differs")
...
def build_directory(destination: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if observer_gate.get("valid") is not True:
        raise BuildError("v8 observer precompile static gate differs")
    ...
    (package / "README.md").write_text(
        "# GAP node0071 v8 dual-ingress localization\n\n"
        "This package is `DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX`. It reuses "
        "the frozen v7 workload/config/golden and adds only qualified "
        "READ_STREAM3-to-Buffer4 and per-operand GA capture counters. "
        "Run once with:\n\n"
        "```bash\n"
    ...
    manifest.update(
        {
            "schema": "gap-node0071-progress-server-package-v8",
            "status": "PACKAGE_READY_NOT_RUN",
            "claim": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX",
            "package_class": "DIAGNOSTIC_ONLY_NOT_FUNCTIONAL_FIX",
            "claim_boundary": (
                "observer-only localization between dual read-stream ingress "
                "and GA operand capture; frozen GAP sum/tail/golden/config/"
    ...
    package_observer["xmr_static_gate"] = observer_gate["xmr_static_gate"]
    binding_contract["source_sha256"] = observer_sha
    ...
    provenance.update(
        {
            "tool":
                "tools/build_gap_node0071_v8_dual_ingress_package.py",
            "bound_source_package_sha256": SOURCE_SHA256,
            "numeric_payload_rebuilt": False,
            "diagnostic_only": True,
            "package_side_change":
                "observer-only qualified dual-ingress localization counters",
        }
    )
...
def main() -> int:
    ...
    try:
        ...
        validation = {
            "schema":
                "gap-node0071-dual-ingress-package-validation-v8",
            ...
            "source_v7_superseded_for_diagnostics": True,
        ...
        print(f"GAP v8 build failed: {error}", file=sys.stderr)
        return 1
