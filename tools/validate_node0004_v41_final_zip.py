...
import tools.validate_node0004_v39_final_zip as v40audit  # noqa: E402


prior = v40audit.prior
previous = v40audit.previous
...
def main() -> int:
    ...
    parser.add_argument("--source-v40", required=True, type=Path)
    ...
    source_path = args.source_v40.resolve()
    ...
    expected_added = {"provenance/diag_reduction_v41.json"}

    previous.INSTALL_NAME = INSTALL_NAME
    previous.FEATURES = FEATURES
    with tempfile.TemporaryDirectory(prefix="v41-final-audit-") as temp:
        root = Path(temp)
    ...
    reduction = json.loads(
        entries.get("provenance/diag_reduction_v41.json", b"{}")
    )
    ...
    checks = {
        ...
        # The plan is explicitly mutable provenance.  A control-plane status
        # update must not invalidate an otherwise byte-identical final ZIP;
        # active rule identities above remain fail-closed/current-match.
        ...
        "v40_adjudication": (
            manifest.get("v40_return_adjudication", {}).get(
                "bound_return_sha256"
            )
            == RETURN_SHA256
            and manifest.get("v40_return_adjudication", {}).get(
                "regression"
            )
            is False
            and "pop without simultaneous push"
            in manifest.get("v40_return_adjudication", {}).get(
                "observer_correction", ""
            )
        ),
        "candidate_matrix": (
        ...
        "observer_hooks_and_feature": (
            observer.count("WRTERM2_FINAL_POP_V1") == 1
            and observer.count("WRTERM2_EDGE_V1") == 1
            and observer.count("WRTERM2_BOUNDARY_V1") == 1
            and observer.count("WRTERM_EDGE_V1") == 0
            and observer.count("WRTERM_BOUNDARY_V1") == 0
            and observer.count(
            ...
            not in observer[observer.index("// v41 WRTERM2") :]
        ),
        ...
        "legacy_occupancy_blocker_not_reintroduced": (
            ...
            not in json.dumps(
                manifest.get("v40_return_adjudication", {}),
                sort_keys=True,
            )
        ),
    }
    ...
    report: dict[str, Any] = {
        "schema": "node0004-v41-final-zip-rule-self-audit-v1",
        "valid": not errors,
        ...
        "source_v40_sha256": prior.sha256_file(source_path),
