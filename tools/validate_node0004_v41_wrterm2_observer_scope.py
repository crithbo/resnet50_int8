...
module rd_buffer_ag_stub;
  ...
  logic buf_ag_ob_wr_ptr, buf_ag_ob_rd_ptr;
  logic [63:0] buf_ag_ob_row_addr [0:1];
  logic [63:0] buf_ag_ob_col_addr [0:1];
endmodule
module memory_ag_idx_queue_stub;
  logic [63:0] mse_mem_queue_bp_pre;
  logic [63:0] mem_idx_valid_same_gotten_masked;
  logic [63:0] mem_idx_valid_bit_masked;
  logic [63:0] mem_idx_same_bit_masked;
  logic [63:0] mem_idx_gotten;
  logic [1:0][63:0] mse_mem_queue_tag;
endmodule
module buffer_ag_idx_queue_stub;
  logic buf_ag_idx_queue_wr_en, buf_ag_idx_queue_full;
  logic buf_ag_idx_queue_rd_en, buf_ag_idx_queue_empty;
  logic [63:0] buf_ag_idx_queue_wr_data;
  logic [63:0] buf_ag_idx_queue_rd_data;
  logic [63:0] mse_buf_queue_row_idx_masked;
  logic [63:0] mse_buf_queue_col_idx_masked;
  fifo_stub u_buf_ag_idx_queue();
endmodule
module memory_wr_stream_engine_stub;
  ...
  buffer_ag_idx_queue_stub u_Buffer_AG_Idx_Queue();
endmodule
module wr_mse_stub;
...
def owner_for(expression: str) -> str | None:
    ...
    if ".u_buf_ag_idx_queue." in expression:
        return "u_buf_ag_idx_queue"
...
def leaf_for(expression: str) -> str:
    match = re.search(
        r"\.([A-Za-z_][A-Za-z0-9_]*)(?:\[[0-9]+\])?$", expression
    )
...
def main() -> int:
    ...
    closure = {
        ...
        "corrected_terminal_predicate_present": (
            "wt_desc_pop && !wt_desc_push" in block
        ),
    ...
    selected = next(
        (
            ...
            if item["leaf"] == "buf_ag_idx_queue_wr_data"
        ),
        None,
    )
    if selected is None:
    ...
    with tempfile.TemporaryDirectory(prefix="v41-wrterm2-scope-") as temp:
        root = Path(temp)
        ...
        bad_predicate = compile_case(
            args.iverilog.resolve(),
            root,
            "negative_terminal_predicate_consumer_typo",
            full_source.replace(
                "wt_desc_pop && !wt_desc_push",
                "wt_desc_pop && !wt_desc_push_actual_typo",
                1,
            ),
        )
    ...
    negatives = {
        ...
        "missing_enable_owner_fail_closed": missing_enable["exit_code"] != 0,
        "terminal_predicate_consumer_typo_fail_closed": (
            bad_predicate["exit_code"] != 0
        ),
    ...
    report = {
        "schema": "node0004-v41-wrterm2-actual-consumer-scope-v1",
        "valid": not errors,
        "errors": errors,
        "rule_id": (
        ...
        "negative_controls": {
            **negatives,
            ...
            "details": {
                ...
                "missing_enable_owner": missing_enable,
                "terminal_predicate_consumer_typo": bad_predicate,
                "task_consumer_typo": missing_task,
            },
        },
        "all_negative_controls_fail_closed": all(negatives.values()),
        "claim_boundary": (
            "The focused wrapper models only exact final HDL external XMR "
            "owners and package-local declarations. It does not elaborate the "
            "full DUT or claim functional correctness."
        ),
