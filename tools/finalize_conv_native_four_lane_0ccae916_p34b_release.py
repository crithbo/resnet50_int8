#!/usr/bin/env python3
"""Create the final release receipt for the frozen p34b diagnostic ZIP."""
...
BASE = ROOT / "outputs/conv_native_four_lane_0ccae916_p34b_armtoken"
BUILD = BASE / "build"
ZIP = BUILD / f"{PACKAGE}.zip"
FILES = {
    "build": BUILD / f"{PACKAGE}.build.json",
    "family": BASE / "p34b_family_audit.json",
    ...
    "p33b_return_analysis": ROOT / "outputs/conv_native_four_lane_0ccae916_p33b_return_analysis/report.json",
    "prior_first_fresh": ROOT / "outputs/conv_native_four_lane_0ccae916_p31_postclear/first_fresh_extra_audit/validation.json",
    "p34_failed_family_audit": ROOT / "outputs/conv_native_four_lane_0ccae916_p34_armtoken/p34_family_audit.json",
}
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError("refusing to overwrite p34b final release receipt")
    if not ZIP.is_file() or not all(path.is_file() for path in FILES.values()):
        raise RuntimeError("p34b final evidence is incomplete")
    ...
    checks = {
        ...
        "p33b_formal_analysis": reports["p33b_return_analysis"].get("valid") is True,
        "p34_unreleased_dependency_escape_retained": reports["p34_failed_family_audit"].get("valid") is False and "target_parser_source_contract_bound" in reports["p34_failed_family_audit"].get("errors", []),
    }
    ...
    matrix = {
        ...
        "arm_token_correlation": {"applicability": "blocking_applicable", "pass": checks["family_audit"]},
    ...
    result = {
        "schema": "conv-native-four-lane-p34b-armtoken-final-zip-audit-v1",
        ...
        "failed_intermediate_disposition": {"p34_zip": "UNRELEASED_SUPERSEDED_PREAUDIT_IDENTITY_RETAINED_NOT_REBUILT", "p34_exact_zip_rebuilt": False, "escape": "package-local ARM parser imported a workspace-only p33 module; p34b embeds all parsing primitives and passes exact-ZIP controls"},
        ...
        "claim_boundary": "One c0 diagnostic correlating exact-target live Buffer5 clear/ARM accepts to ARM token counters/reset/last/same state. No natural terminal, formal 320D, E3/E4/E5 or performance claim.",
        "server_action": False,
    }
