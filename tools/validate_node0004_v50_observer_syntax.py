...
def main() -> int:
    ...
    source = (
        ...
        f"{declarations}\n{normalized}\n"
        "  initial begin #1; "
        'return_obs_write_dterm_owner_state("FOCUS"); end\nendmodule\n'
    )
    with tempfile.TemporaryDirectory(prefix="v50-dterm-syntax-") as temp:
        root = Path(temp)
        ...
        task_typo = compile_case(
            ...
            "negative_task_typo",
            source.replace(
                'return_obs_write_dterm_owner_state("FOCUS")',
                'return_obs_write_dterm_owner_state_typo("FOCUS")',
                1,
            ),
        )
    ...
    report = {
        "schema": "node0004-v50-dterm-observer-syntax-v1",
        ...
        "claim_boundary": (
            ...
            "exact v50 changed span; hierarchy binding is checked separately."
        ),
    }
