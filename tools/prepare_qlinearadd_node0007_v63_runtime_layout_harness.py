#!/usr/bin/env python3
"""Bind a six-scenario structural runtime-layout harness to exact QAdd v63 ZIP."""
...
def main() -> int:
    ...
    for index, (name, code) in enumerate(exit_codes.items(), start=1):
        stamp = f"17880000000000000{index:02d}"
        result = f"/home/panqs/ndp/simresult/{PACKAGE}_r{stamp}_{2000 + index}_return.zip"
        scenarios[name] = {
            "command": f"STRUCTURAL_LOCAL_EXACT_ZIP scenario={name} bash {PACKAGE}/PREPARE_AND_RUN.sh /synthetic/NDP_copy01",
            "cwd": "/synthetic/NDP_copy01", "runner_exit": code,
            "compile_started": name not in {"preflight_fail"},
    ...
    report = {
        ...
        "scenarios": scenarios,
        "claim_boundary": "Exact-final-ZIP static six-exit structure combined with independent runner, post-sim and TB-VCD state-machine gates; no server, VCS or DUT action.",
    }
