...
BUILD_ID = "r5-node0071-node0075-e1fb0f7-native-ordering-package-v5"
...
OBSERVER_SOURCE = (
    ROOT
    / "tests/rtl/"
    "node0071_node0075_e1fb0f7_native_ordering_observer_v4.svh"
)
V3_RETURN_ANALYSIS = (
    ROOT
    / "artifacts/operator_config_validation/"
...
def _build_tree(root: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "successor_of": {
            ...
            "return_classification": "PACKAGE_LOCAL_DELIVERY_SELF_AUDIT_ESCAPE",
            "first_divergence": (
                "native_return_observer.svh:211 bare clk_sg and line 218 "
                "bare rst_n_sg were unresolved in the actual TB scope"
            ),
            "repair_scope": (
                "package-local observer SG clock/reset hierarchy binding and "
                "fresh delivery identity only"
            ),
            "causal_materialized_semantics_changed": False,
            "package_namespace_sca_path_bytes_changed": True,
        },
        "supersedes_held_identity": {
            "package_name": "r5_n71_n75_e1f_native_v4",
            "status": "HELD_RULE_DRIFT_BEFORE_RELEASE",
            "reason": (
                "current config/server rule receipts changed after the v4 "
                "deterministic build and before final release"
            ),
        },
        "materialized_config_rule_applicability": {
            "changed_surface": "PACKAGE_NAMESPACE_ONLY",
            "causal_transaction_ledger": {
                "rule_id": "CDA-CONFIG-CAUSAL-TRANSACTION-LEDGER-001",
                "applicable": False,
                "basis": "v3 frozen materialized causal slice exact-byte reuse",
            },
            "boundary_microtrace": {
                "rule_id": "CDA-CONFIG-BOUNDARY-MICROTRACE-001",
                "applicable": False,
                "basis": "no changed threshold/mask/capacity/lifetime/CONFIG predicate",
            },
        },
        "single_command": (
            "bash PREPARE_AND_RUN.sh /absolute/path/to/NDP_copyXX"
