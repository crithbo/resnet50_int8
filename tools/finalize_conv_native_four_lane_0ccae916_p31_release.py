#!/usr/bin/env python3
"""Create the final release receipt for the frozen p31 diagnostic ZIP."""
...
FILES = {
    ...
    "family": BASE / "p31_family_audit_v2.json",
    ...
    "first_fresh_contract": BASE / "first_fresh_extra_audit/contract.json",
    "first_fresh_validation": BASE / "first_fresh_extra_audit/validation.json",
    "preparation_escape": BASE / "first_fresh_extra_audit/preparation_escape.json",
    "p30_return_analysis": ROOT / "outputs/conv_native_four_lane_0ccae916_p30_return_analysis/report.json",
}
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError("refusing to overwrite p31 final release receipt")
    if not ZIP.is_file() or not all(path.is_file() for path in FILES.values()):
        raise RuntimeError("p31 final evidence is incomplete")
    ...
    checks = {
        ...
        "prebuild_once": reports["profile"].get("contract_valid") is True and reports["profile"].get("aggregate_prebuild", {}).get("top_level_invocations") == 1,
        "first_fresh_extra_audit": reports["first_fresh_validation"].get("pass") is True and reports["first_fresh_validation"].get("upload_authorized") is True and reports["first_fresh_validation"].get("package_id") == PACKAGE,
        "p30_formal_analysis": reports["p30_return_analysis"].get("valid") is True,
        "audit_escape_content_neutral": reports["preparation_escape"].get("exact_final_zip_changed") is False and reports["preparation_escape"].get("final_validator_invocations_before_escape") == 0,
    }
    ...
    matrix = {
        ...
        "first_fresh_extra_audit": {"applicability": "blocking_applicable", "pass": checks["first_fresh_extra_audit"], "epoch_id": "20260810-first-fresh-extra-audit-v1"},
    ...
    result = {
        "schema": "conv-native-four-lane-p31-postclear-final-zip-audit-v1",
        ...
        "claim_boundary": "One c0 diagnostic distinguishing final same-row2 reach and post-clear bank-ready 00/0f/f0/ff/other. No natural terminal, formal 320D, E3/E4/E5 or performance claim.",
        "server_action": False,
    }
