...
import tools.validate_node0004_v32_runner_controls as prior  # noqa: E402
...
def write_stubs(stub_root: Path, python: Path) -> None:
    ...
    anchor = (
        ...
        "feature=RETURN_OBS_MSE4_INDEX enabled=1 "
        "limit_name=RETURN_OBS_MSE4_INDEX_LIMIT limit=96\n"
    )
    addition = anchor + (
        "0 | DIAGNOSTIC_FEATURE_ENABLE_V1 | "
        "feature=RETURN_OBS_LC18_PE7 enabled=1 "
        "limit_name=RETURN_OBS_LC18_PE7_LIMIT limit=96\n"
        "0 | DIAGNOSTIC_FEATURE_ENABLE_V1 | "
        "feature=RETURN_OBS_ROWLC4_BUFAG enabled=1 "
        "limit_name=RETURN_OBS_ROWLC4_BUFAG_LIMIT limit=128\n"
    )
...
def main() -> int:
    ...
    report = base.validate(
        ...
        report_schema="node0004-v35-runner-controls-v1",
