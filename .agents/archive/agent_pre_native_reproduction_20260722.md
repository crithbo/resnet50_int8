# 已归档：ResNet50 INT8 旧项目总览（2026-07-22 原生复现路线之前）
...
稳定门快照：W0/G0、W2/G2、W3/G3、W4/G4已通过；W1仅冻结当前可取得的模型/输入/量化事实，G1未整体关闭；`node-0004` Conv的正式编码、config-bound软件P/D和freeze-bound单算子运行包已闭合，`node-0002` MaxPool也已完成正式编码及config-bound软件A/D，因此这两个实例范围G5=true。其他shape/算子族的G5仍未闭合，目标数值执行和RTL三方证据尚无，因此G6/G8保持false。最新变化以`plan.md`为准。
...
| `NDPFuncModel/conv_func`及`component/GeneralPEA.py` | Conv与MaxPool配置绑定功能模型、physical staging/inverse和P/D或A/D比较 | 逐周期RTL、bitstream解释器 |
...
- `tools/generate_conv_1x1_real.py`、`tools/generate_conv_3x3_real.py`、`tools/generate_conv_1x1_requant_real.py`：按实例几何生成current ABI accumulate/per-channel requant配置；3×3路径显式绑定pad1 halo与HIGH-4合同。
- `tools/run_conv_1x1_encoder.py`、`tools/run_conv_1x1_requant_encoder.py`：调用正式parser/placement/encoder并复验确定性。
- `tools/prepare_conv_server_profile_template.py`：从typed request、物理地址计划和mapping seed生成内容寻址的初始server profile；原生prepare仍必须刷新并复验最终合同。
...
- `tools/audit_senior_conv_operator.py`、`tools/audit_senior_conv_bundle.py`：隔离复现学长3×3伪代码的原始placement失败、受控结构修复双编码、ResNet节点匹配和有边界的NDP数值探针，并生成/独立审计明确不可作为服务器overlay的encoder诊断包。
- `tools/generate_maxpool_instance.py`、`tools/run_maxpool_encoder.py`、`tools/run_w5_maxpool_preflight.py`：生成`node-0002/hwop-0002-00`三轮28-slice MaxPool配置，调用正式encoder，并完成W3 Golden与配置绑定GeneralPEA的logical/physical A/D比较。
- `tools/verify_maxpool_rtl_uint8.py`、`tools/audit_maxpool_target_attempt.py`：分别保存活动RTL无符号逐字节max的穷举算术证据，以及完整target交付链缺失时的fail-closed边界；前者不是整算子target执行。
- `tools/generate_native_json_maxpool_package.py`、`tools/analyze_native_json_maxpool_return.py`：生成并验证`node-0002`两份真实channel tile的native-JSON服务器控制包，并在服务器回传后重组4段readback、逐byte比较W3 golden；该最小包不等于batch16/28-slice整实例已执行。
...
1. 当前Conv v19已因真实run1停滞永久撤权；下一可执行候选是`node-0002` MaxPool最小控制包，先完成其服务器自然结束、完整readback和两tile适用范围三方比较。最小包通过仍不能外推batch16/28-slice或整网G6/G8。
