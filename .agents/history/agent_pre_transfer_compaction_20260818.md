...
最后更新：2026-08-18（新会话接手与构包 Skill 触发说明）
...
前，必须从“生成前必读索引”选择本轮相关资料并保存路径/用途读取收据。本文件不复写这些规则。

活动规则的 exact-set、分层、职责与读取 profile 由
...
以下动作必须调用项目 Skill `.codex/skills/resnet50-server-package-flow/SKILL.md`：向注册 family
派发任务；生成或修补服务器包、runner、observer、可选TB VCD或return；分析正式return；设计
successor；审查构包事故。纯数值/config研究且不触碰服务器包时不调用。主线不得用临时子代理
替代owner registry中的持久family会话；Skill未被环境自动发现时也必须显式读取该路径。
Skill只编排current registry、聚合检查和回执；活动规则与机器validator仍是权威。

任何全新会话或其它模型接手时固定执行五步：

1. 完整读取本文件和current `.agents/plan.md`，不要先读历史task record；
2. 从`contracts/current_session_owner_registry_v1.json`解析自己的稳定`role_id`、唯一ACTIVE owner
   与current mainline；
3. 按下表只读共同必读和本角色增量，再从current-family pointer/latest record恢复in-flight事实；
4. 命中上述服务器包触发条件时立即调用项目Skill；其它任务只按生成前索引选读；
5. 先报告`previous proven progress / current purpose / write scope / forbidden actions / conflicts`，
   再继续原任务。历史thread ID只作provenance，不能获得或转移权限。
...
`生成前必读索引.md` 对 changed surface 做一次增量选择并记录路径/用途；SHA不是读取完成条件。
...
稳定语义时，先明确唯一 owner，并用
`tools/validate_rule_maintenance_incident_adjudication.py` 把事故分类为规则语义错误、真实语义遗漏、
实现逃逸、会话执行不合规或单次领域/环境失败。只有前两类修改规则；实现逃逸只修共享硬门，
会话执行问题先修 Skill、handoff 或适用性门，且仅在能映射到
`server_start/actual_input/state_safety/return` 时增强 blocking gate。规则修改允许替换、合并、
收窄、删除和归档，禁止把“每次事故追加一条”作为默认动作；修改后刷新 registry，禁止复制同义
段落到多个专项规则。
...
- tested包、原始回传和已有evidence不覆盖；尚未执行的本地/pending包优先修补当前revision并复用
  未变资产与已PASS门，已执行或绑定正式return后才使用fresh successor。禁止逐错误完整重建。
...
生成器、validator、实例合同、artifact 和 task record。开始工作前必须读取派发任务指定规则；
只有actual input consumer明确需要时才把digest作为阻断条件。
...
主线派发任务单至少包含目标、适用规则、冻结/可复用资产、changed surfaces、通过口径、失败路由、允许写入范围和
...
在同一任务中完成receipt-only分析并继续到下一包裁决。本地读取、分析、修补未执行revision或
生成已执行包的fresh successor，以及执行受影响的final-ZIP因果自检属于既有任务默认权限。
...
1. 若现有 return、冻结配置、execplan/SCA、消费者和相关 RTL 已能确定根因，且修复仅涉及
   config、runner、package-local observer、validator 或封包基础设施，则未执行包优先原revision
   patch并只重跑changed-surface门；已执行包才使用fresh identity生成修正包；
