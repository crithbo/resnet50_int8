...
node0007 split-C v29 是冻结反例：op_fp32_add 的 Buffer0/Buffer2 各只有一笔16B
qualified accepted write，而 ARM 对8个 bank 的32B row mask 全部有效；两侧
`buf2arm_rreq_ready` 与 ARM accept 均保持0，GA operand capture为0。合法候选须在同一
32B物理row内提供两个不重叠窗口 `[0,16)`、`[16,32)`，但仍须由 fresh 最终物化和动态
return 验收，不能因本节文字直接升级。
