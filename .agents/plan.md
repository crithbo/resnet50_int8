...
- **正在推进**：W4已完成Quantize/Dequantize/View的16-slice candidate正逆布局、坐标解释、校验和manifest记录；Conv、Pool、Add、AvgPool、MatMul仍待实现。W5目标JSON/bitstream、W6目标simulator、W7网络execplan、W8硬件、W9通用三方比较尚未开始。
...
| W4 | 进行中/G4未通过 | Quantize/Dequantize/View candidate已通过软件round-trip | 继续Conv，再按顺序扩Pool/Add/AvgPool/MatMul |
...
当前进度（2026-07-12）：`w4_batch_slice_candidate_v1`已覆盖QuantizeLinear/DequantizeLinear的A、scale、zero_point、D，采用16-slice batch一项一slice、标量qparams逐slice复制、C-order/little-endian和16-byte区域对齐；`w4_zero_copy_view_candidate_v1`已证明正式Flatten `[16,2048,1,1]→[16,2048]` 在同一物理base上零拷贝。最小shape、正式Quantize输入shape、1000类tail和错误注入测试通过。两合同仍为candidate，Conv及其余算子未完成，所以G4保持未通过。
...
当前状态：DeepSeek relayout仍只覆盖28-slice LLM，旧`relayout_layer0.py`、`conv_layout.py`和`layout_buffer.py`不能直接作为ResNet实现。W2已修复NDP slice/transaction寻址并验证1/4-slice小Conv candidate：activation按C、weight/output按K分片，含16-byte对齐、padding/tail、逐字节provenance和bit-exact inverse；但它不是正式16-slice硬件layout。W4已新增Quantize/Dequantize/View的16-slice candidate并通过最小/正式/tail round-trip；Conv、MaxPool、QLinearAdd、GlobalAveragePool和MatMul/dense仍需完成，并用外部layout/端口合同裁决候选规则。
