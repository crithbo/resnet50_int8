...
- 根仓库和子仓库每个经过测试确认有效的小步骤做原子Git提交；W1/W2等工作包通过验收门后推送GitHub里程碑。子仓库提交永久保留，不通过reset/rebase/filter/强推或裁剪历史释放空间。
- 子仓库提交和本地副本永久保留。大阶段确认无误后如空间占用过大，可清理范围仅限主仓库的额外副本/备份，并须确认不是当前工作仓、恢复路径已验证、lock/history完整且操作者再次批准具体绝对路径；当前工作主仓和任何子仓库副本不得删除。
