...
- **下一主线**：先完成W4/G4综合门审计与Conv/整体profile裁决，不自动跳过未通过的G4进入W5；同时继续追踪W1硬件合同。不得重跑约951 MB的W3产物，除非合同/hash/回归明确失效。
...
- **接手检查**：依次运行 `git status --short`、`.venv\Scripts\python.exe tools\sync_repositories.py verify`、`.venv\Scripts\python.exe -m unittest discover -s tests -v`；预期根工作树干净、三参考仓匹配lock、67项测试通过。始终使用根目录 `.venv\Scripts\python.exe`。
...
- **数据变换——W4软件candidate覆盖完成/G4未通过**：Quantize/Dequantize/View、正式20类Conv shape、MaxPool、QLinearAdd、GlobalAveragePool和MatMul/dense均已实现。正式MatMul `[16,2048]×[2048,1000]→INT32 P/UINT8 D [16,1000]`在batch/ring下11端口inverse bit-exact；batch Quantize D→A及两profile MatMul D→dense Add A exact alias成立，ring输入和ring最终Dequantize边界显式要求relayout。ADR-002仍等待Conv/整体拓扑裁决，故不能宣布G4通过。
...
严格按 `.agents/plan.md` 的W0→W9工作包和G0→G9验收门推进。W0/G0、W2/G2、W3/G3已经完成；W1模型部分完成但G1未通过。W4软件candidate已覆盖计划内全部算子，下一步是G4综合门审计与profile裁决，而不是直接宣称W4/G4完成。任何W1～W3修改都必须先说明会使哪些manifest/hash/下游产物失效。
