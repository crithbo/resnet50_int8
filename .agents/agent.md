...
- **当前成果**：正式图含78节点/617张量，lower为133个语义hw_op；保存79个运行时tensor和55个INT32内部tensor，全部78节点独立公式重放匹配ORT，旧77原语已逐项映射。W4-28的C0/C1/C2/C3软件候选工作已完成：14个RTL28 candidate layout覆盖全部七族；两种整网调度完成93边、91 qparam链、16残差Add和79 tensor生命周期/alias审计，并生成静态成本。配置权威审计盘点42个JSON，已完成两个MaxPool和一个AvgPool的字段/寄存器/bitstream审计并冻结Pool族五段联动规则；旧16-slice物理证据只作历史参考。
...
- **下一主线**：等待硬件批准期间，按ADR-008继续审计Quantize与Add-Dequant两个GA模板，重点冻结dtype转换、固定常量和真实ResNet qparams之间的缺口；随后再审计GEMV/MatMul与sum组，并建立W3 hw_op/qparams到正式配置字段的参数化adapter。没有G4授权时只产生审计临时产物，不生成正式W5网络JSON/bitstream。外部证据收到后仍单线程执行批准合同导入和G4重审。
