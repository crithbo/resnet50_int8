...
- **当前可直接推进**：W0/G0、W2/G2已通过；W3已有78节点/617张量正式图目录、133个语义hw_op和正式batch16全部node output，79个运行时artifact重放hash一致。当前继续生成55个lowering内部tensor的subop golden并对照旧77原语；目标JSON/硬件合同仍属G1阻塞。
