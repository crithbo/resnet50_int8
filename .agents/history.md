...
## 2026-07-15：E1第二个真实1×1多K与requant候选闭环

- NDPFuncModel业务提交`e35b24a446bdaeb7a939ab50d8e0cad5fe2a393c`，父提交`1d3181d832d7a409af779215e4aa590d03bd8ed3`，`feat: parameterize conv target bindings`；根仓业务提交`51d6e787e1c4df1fa617a4a2aa3b0ffa0dfcdb46`，父提交`7ea130552f10fc83397a6133f551e9e37d742af2`，`feat: close second real 1x1 candidate`。两仓均只创建本地提交，没有推送云端；根仓提交使用path-limited commit，开工前已暂存的bootstrap/参考模型支持文件保持在原暂存区，没有混入E1。
- `node-0008/hwop-0008-00~01`来自同一typed contract与W3正式tensor：`A=[16,256,56,56]`、`B=[64,256,1,1]`、`P/D=[16,64,56,56]`，`c_tile=64`、`k_tile=16`、8个requant shard。新增通用实例生成、正式encoder和candidate preflight入口；累加JSON、语义合同、encoder证据、requant manifest与8份JSON均按`configs/conv/hwop-0008-00/`隔离，报告位于`artifacts/w5/hwop-0008-00/preflight.json`，不会覆盖首例freeze目录。
- 正式累加encoder两次运行均为46连接、placement cost 0、mapping cache key `66245288c5ee2398`且逐输出一致；mapping review SHA为`2088dd033e7b92c17bd82b065611015a49ef968fccf86c67269e8b911e3ad272`，parsed dump为`1fbe11a7e6c1b0dedbd167f571e0602ff933e5a03cb3ed2d8c2397902df61ee7`，64/128位bitstream分别为`2f82a0ec6ee61bdae53aa5ca81b74490c5d1f7d9839fe12bd16c51844b642c17`和`3f8606b7c7aeeafdb4f74e13d17a13dd36a0bc05c7e3a9923cf1fbf77278f500`。8个requant shard各21连接、cost 0并双次确定；placement连接数与cost未因实例数值改变而漂移。
- NDP request与校验不再写死首例offset/stride/channel：累加循环和stream从实例几何推导；requant严格消费manifest原文/SHA并验证64通道唯一覆盖、8 shard、GA常量、HIGH-ring slice、16B地址、实例循环、唯一flush及两份staging D基址。四段K生命周期按HIGH-ring遍历各覆盖64通道，bias只在first初始化、末段才requant/flush；单坐标、首tile 150,528元素和全算子3,211,264元素的P/D均0 mismatch且actual/golden SHA相同，两份staging D均inverse回canonical D，28个physical writeback全部通过。
- 受控文件SHA：累加JSON `f1b8478a8a210903867c7d5c04a9b682cc2ec4752850ebd505c80050b64e243b`；语义合同`a46ba8224bfb2e22f05521cdcc61d38d3d3519c6aea6143740f2cf1c70fe5bc0`；encoder证据`146d4ba1f056ac748567bc91a8e1c92ff2485714377c760dd6543715a3d89ba9`；requant manifest `3cfc892f9947882afb86a97499c725bbd04ac65fca536a90c30b9fb3e649cf8e`；candidate preflight `6e30380698252f366eb1c0931553fc26b6bc0437f0bb2728aceb40f906fc3ded`。首例当前preflight因NDP源码提交和backend合同身份更新为`e1143e815a15e51ef97a7a4ea84b260b7081c7949f83173aa06c5e52db7e28a4`；其数学P/D、配置和bitstream未变，硬件组持有的freeze manifest仍为`72e17cb52c2948f86fe6b0e9b2715de57c5404a72a04f9514247f174e8a95550`，包内旧preflight也未改写。
- 首次完整运行暴露两个真实软件扩展问题并已闭合：固定60秒NDP预算对4倍MAC实例不足，现按实例MAC缩放且首例回归预算提高到120秒；根adapter曾写死首例`channel_count/shard_count/staged_d_offsets`，现从请求manifest严格推导期望值。二者都不是P/D数值偏差。新增负向测试拒绝配置SHA/coverage/flush/地址/GA漂移及硬件门越权；最终根仓258/258、NDPFuncModel 20/20、`tools/sync_repositories.py verify`和`git diff --check`全部通过。
- E1只形成`candidate_config_and_config_bound_pd_passed`：hardware/G5/G6/G8仍为false，`B_EXECPLAN_TYPED_TRANSPORT`仍存在。下一串行工作包是E2 `node-0003`的64→256输出通道、32 shard与多staging/inverse扩展；E2/E3冻结公共schema后，E4才允许按独立实例目录并行。操作者两份未跟踪伪代码原文保持未修改、未暂存、未提交。
- 精确回退：先revert后续说明台账提交，再在根仓`git revert 51d6e787e1c4df1fa617a4a2aa3b0ffa0dfcdb46`回到`7ea130552f10fc83397a6133f551e9e37d742af2`；NDPFuncModel单独`git revert e35b24a446bdaeb7a939ab50d8e0cad5fe2a393c`回到`1d3181d832d7a409af779215e4aa590d03bd8ed3`。回退不得清理既有暂存文件、两份操作者伪代码或首例硬件freeze目录。

## 2026-07-15：E2输出通道扩展1×1与32-shard闭环

- NDPFuncModel业务提交`9004ff73e2e2d7c501f682de6df8543a45ae56cc`，父提交`e35b24a446bdaeb7a939ab50d8e0cad5fe2a393c`，`fix: admit wide conv shadow image`；根仓业务提交`2ad29d247a296679d248e3923a7fadbccd23f8f3`，父提交`3727b0a4d953c65ac7eb910ab529b36e3b51964e`，`feat: close wide-output 1x1 candidate`。两仓均只创建本地提交、没有推送；根仓继续使用path-limited commit，既有bootstrap/参考模型支持文件仍留在原暂存区且未混入，两份操作者伪代码未修改、未暂存。
- `node-0003/hwop-0003-00~01`从同一typed contract/W3数据生成：`A=[16,64,56,56]`、`B=[256,64,1,1]`、`P/D=[16,256,56,56]`，`c_tile=16`、`k_tile=64`、32个8-channel requant shard。受控SHA为：累加JSON `a9e014ccf057eb877a2f259cbb8d5c014fa1eed54db835dadb028286425659b5`，语义合同`d873e36abfcf275b5b8ed25157de7e3492772d9e5c1612b185119633e1336983`，累加encoder证据`77db8e367bb7d4e4be549ec61daf5830ff16e8f0e129fea9e7c624e8c53cec84`，requant manifest `485dabb8e5f7f9d7c70b2398174c3b398b45e3eebb608b546a2917dc70b11552`，candidate preflight `19eb15b48c58e78973a438ca9e05f222a74e961d6552a1ae778d05e789be9f60`。
- 正式累加encoder两次均为46连接、placement cost 0、mapping key `66245288c5ee2398`，mapping review SHA仍为`2088dd033e7b92c17bd82b065611015a49ef968fccf86c67269e8b911e3ad272`；parsed、64/128位bitstream和detailed SHA分别为`637a654484a974c7cb2aa1005117ae9aef85ee2bdf73056cb4e1a1aaeba726b5`、`460b6be181ccd7345091d91ae75a4b274b5dba5a1e4b8cabf4e2792f513cb14e`、`43b05ebc3fb053ae04ecafba99a44345aa56527c26831fa00f8325dbcce1e265`、`84c76d1e465aad6bcbe85de064cb99ecf0604d2c440076d45b25f7aa059bd563`。32个requant shard全部由正式encoder双次运行，各21连接/cost 0且逐输出一致；wrapper默认config/output根已改为从`node_id/request`推导，不再拿新实例与首算子目录比较或覆盖。
- NDP config-bound结果：单坐标、首tile 602,112元素、全算子12,845,056元素的P/D均0 mismatch且actual/golden SHA相等；256通道覆盖集合恰好为`[0,256)`，32 shard无重复/缺失，每个HIGH destination由8个本地shard写8份staging D，28个physical writeback全部inverse回canonical D且每逻辑输出只flush一次。另固定抽查`[0,0,0,0]`、`[0,128,0,0]`、`[0,255,0,0]`：P分别为3251/-2923/2218，D分别为154/147/151，均与W3相同并保存P/D物理地址。
- E2过程中按正式工具先后暴露并关闭四个扩展硬编码：requant encoder默认指向首例目录；首tile检查固定`logical_count=16`；NDP探针总shadow上限固定64 MiB；RTL28→NDP request每destination只列两份staging。修复后路径由request隔离、tile使用实例`k_tile`、staging基址数量使用`requant_shards_per_owner`；目标plan每slice只使用3,165,824 B/容量25,165,824 B，因此约105.5 MB是28-slice compact shadow进程总量而非硬件超容，NDP仍保留128 MiB显式安全上限并新增回归测试。
- lock更新到`NDPFuncModel@9004ff73e2e2d7c501f682de6df8543a45ae56cc`后，E1报告重生为`b08804a644ff66daea84d9b3f96fda30dcc6b06392efce3422c5ca3da36384d5`并新增通道0/32/63样本；首算子当前preflight仅因NDP源码身份/backend SHA更新为`abae81ff16d990d9caabc263ebf768b2be21ad8652634863be6f4824ee3c6bd8`。首例JSON、requant、bitstream、P/D及硬件组持有的freeze manifest `72e17cb52c2948f86fe6b0e9b2715de57c5404a72a04f9514247f174e8a95550`未改写。
- 最终根仓260/260、NDPFuncModel 21/21、两实例生成器`--check`、首例requant encoder兼容性、`tools/sync_repositories.py verify`和`git diff --check`全部通过；仅有既有NumPy弃用告警。E2仍只批准`candidate_config_and_config_bound_pd_passed`，hardware/G5/G6/G8保持false，`B_EXECPLAN_TYPED_TRANSPORT`仍是自动扩展阻塞；当前单线程进入E3，E3冻结公共schema/API后才能开启E4实例级并行。
- 精确回退：先revert后续说明台账提交，再在根仓`git revert 2ad29d247a296679d248e3923a7fadbccd23f8f3`回到`3727b0a4d953c65ac7eb910ab529b36e3b51964e`；NDPFuncModel单独`git revert 9004ff73e2e2d7c501f682de6df8543a45ae56cc`回到`e35b24a446bdaeb7a939ab50d8e0cad5fe2a393c`。若还需回退E1，再按上一条记录继续；不得清理既有暂存文件、两份操作者伪代码或首例hardware freeze。

## 2026-07-15：E3 Conv typed execplan transport闭环

- 正式`ndp-sim-ref/model_execplan`先在`ef6dd1c668caf39482ce759313d2f9e051bd22eb`扩展schema与consumer：`TensorSpec`增加tensor identity/logical shape，`OperatorSpec`增加instance/stage/attributes、`TypedConstantSpec`和`ConfigArtifactSpec`；loader严格保留dtype、shape、axis、原值SHA、FP32 bit pattern、派生公式、目标字段绑定，以及配置JSON UTF-8原文/SHA。`control_registers.py`只消费显式`control_register:` binding，不再从整数`params`或文件名猜scale/channel。官方3个round-trip/绑定/负向测试通过。
- W4硬件批准合同仍绑定上游README原始字节，因此随后用`5aa7d44aa2c58b7c86cfa32389d135246001629c`恢复README为批准基线SHA-256 `992360e8...`，只保留正式代码和测试变化。根仓`repos.lock.json`锁定当前execplan提交`5aa7d44...`，同时`source_versions.py`要求不可变JSON/encoder基线`e299b280...`是其祖先且工作树干净；这避免把“更新execplan”误写成“迁移正式配置基线”。
- 根仓新增`conv_execplan_transport.py`和统一CLI `tools/generate_conv_execplan_request.py --node-id ...`。request含同一Conv instance ID下的accumulate/requant两个operator，直接从正式ONNX initializer取得bias、scale、zero-point及值SHA，并携带累加配置/语义原文、requant manifest及全部shard JSON原文/SHA。NDP严格复用现有配置绑定校验，覆盖28 slice、64/256输出通道、GA常量、HIGH ring、16B staging、LC和唯一flush。
- 三份受控request均由同一CLI只改`node_id`生成并通过parse→round-trip→NDP验证：first `fd6efbe86193f5d9660e8ae746763632b29ff8c968e10415d666a62f22ed3fe8`/226,800 B，E1 `467fe1985f2971fe2be13f715cc81f8a9fbf35d930db612498e787173117177f`/230,183 B，E2 `48a793ed0710c3a931318deab549cbf0d4be46eee32c93a2d579d149574a690d`/785,122 B。对应preflight为`dff41e18e324d109257bb4f4d425ec251448c98efc526a987c243dfe779c5f46`、`3f284ed6ae6df63ea12c7210942da51bc3d8a264a121e85863107c9e270d8ad2`、`e4777174c1a0b0e992fc3e070540e3bb8c4b8195d5a0c77187de1ed75a460ba3`；重新执行三档P/D仍通过，候选hardware/G5/G6/G8保持false。
- 新W5证据`contracts/target_execplan_transport_audit.json` SHA-256为`1a4af25b1860093fff084eac8804fe5c060c6b5c8538ba491a2b155338e25300`，闭环合同`contracts/conv_execplan_transport.json` SHA-256为`4f4e8b4e0d4ea7e2f7d2a506047c14154d5a55606f055baa4b6d96bfa6cabc75`。缺scale/zp/bias、axis丢失、float截断、旧16-slice字段和JSON SHA漂移均在执行前失败；`B_EXECPLAN_TYPED_TRANSPORT`因此从当前阻塞清单删除。
- 为避免篡改历史批准证据，本包没有重写W4快照：`target_config_authority_audit.json`仍为`b814362...`，`typed_config_parameter_contract.json`仍为`abbc87b...`，backend和ADR/hardware approval字节不变；首例freeze ID `f687debd...`及manifest `72e17cb...`也保持只读。新的E3结论只写入独立W5合同和三实例preflight。
- 验证结果：根仓264/264、NDPFuncModel 21/21、官方execplan 3/3、三份request `--check`、仓库lock、W4 gate和`git diff --check`全部通过；只出现既有NumPy弃用告警。根仓业务提交`f4dfe4b83efa442bc8c201bf5f8e2bc71636b124`，父提交`c93d957...`，`feat: close Conv typed execplan transport`；本轮没有推送。既有暂存的环境/W3文件和操作者`.agents/conv_full(2).json/.txt`未混入提交。
- 当前边界：typed字段运输已经闭合，但这不等于单算子运行包或W7整网闭合。下一串行包E3b要把首例两个逻辑阶段展开为1个accumulate加8个requant运行时算子，经正式`model_execplan`生成无golden预加载的`execplan.txt/Bank_data/sca_cfg`并审计mask、地址、阶段和hash；E3b通过后才允许E4按互斥目录实例级并行。
- 精确回退：先revert后续说明台账提交，再在根仓执行`git revert f4dfe4b83efa442bc8c201bf5f8e2bc71636b124`回到`c93d957...`；`ndp-sim-ref`单独先revert`5aa7d44aa2c58b7c86cfa32389d135246001629c`、再revert`ef6dd1c668caf39482ce759313d2f9e051bd22eb`回到`e299b2804448242d1589b3e58ed7c5a9a5eca09f`。回退不得改写W4批准快照、hardware freeze、既有暂存文件或两份操作者伪代码。
